...
static int jadard_report_data(struct jadard_ts_data *ts, int ts_path, int ts_status)
{
    ...
    if (ts_path == JD_REPORT_COORD) {
        /*A14 code for SR-AL6528V-01-144 by renbulang at 20240914 start*/
        if (get_tp_enable()) {
            g_module_fp.fp_report_points(ts);
        }
        /*A14 code for SR-AL6528V-01-144 by renbulang at 20240914 end*/
#ifdef JD_SMART_WAKEUP
...
static const struct proc_ops jd_info_proc_fops = {
   .proc_read = jd_proc_getinfo_read,
};
...
static int tp_usb_check_notify_callback(struct notifier_block *nb,unsigned long event,void *ptr)
{
...
/*o22 code for SR-AL6528V-01-137 by lihao at 20240911 start*/
static void jadard_get_tp_module(void)
{
    struct jadard_ts_data *ts = pjadard_ts_data;

    if (tp_choose_panel("jd9522t")) {
        strcpy(ts->module_name,"JD9522T_XX_BOE");
    } else {
        strcpy(ts->module_name,"unknown");
    }
}

int jd_ito_test(void)
{
    JD_I("%s():Unrealized functions",__func__);
    return 0;
}
...
{
    struct jadard_ts_data *ts = pjadard_ts_data;
    struct lcm_info *gs_jd_lcm_info = NULL;

    gs_jd_lcm_info = kzalloc(sizeof(*gs_jd_lcm_info), GFP_KERNEL);
    if (gs_jd_lcm_info == NULL) {
        kfree(gs_jd_lcm_info);
        JD_E("%s: gs_jd_lcm_info kzalloc fail, and is NULL!\n", __func__);
        return;
    }

    gs_jd_lcm_info->module_name = ts->module_name;
    gs_jd_lcm_info->sec = ts->sec;
    gs_jd_lcm_info->fw_version = &(pjadard_ic_data->fw_cid_ver);
    /*gesture wakeup*/
    gs_jd_lcm_info->gesture_wakeup_enable = jadard_enable_gesture_wakeup;
    gs_jd_lcm_info->gesture_wakeup_disable = jadard_disable_gesture_wakeup;
    /*ITO TEST*/
    gs_jd_lcm_info->ito_test = jd_ito_test;
    JD_E("%s:module_name=%s\n", __func__, gs_jd_lcm_info->module_name);

    tp_common_init(gs_jd_lcm_info);
}
/*o22 code for SR-AL6528V-01-137 by lihao at 20240911 end*/
...
/*hs14 code for AL6528A-169 by liudi at 20220928 end*/
    ...
    err = register_usb_check_notifier(&ts->tp_usb_notify);
    if (err) {
        JD_E(" Unable to register tp_usb notifier: %d\n", err);
    }
#endif
#ifdef JD_EARPHONE_DETECT
    ts->earphone_notify.notifier_call = jadard_earphone_notifier_callback;
    err = headset_notifier_register(&ts->earphone_notify);
    if (err) {
        JD_E(" Unable to register earphone notifier: %d\n", err);
    }
#endif
...
/*o22 code for SR-AL6528V-01-137 by lihao at 20240911 start*/
    JD_I("%s: start tp_feature_interface\n",__func__);
    jadard_get_tp_module();
    tp_feature_interface();
/*o22 code for SR-AL6528V-01-137 by lihao at 20240911 end*/

    /*hs14 code for SR-AL6528A-01-488 by liudi at 20220914 start*/
    err = sysfs_create_link(&ts->sec.fac_dev->kobj,&ts->input_dev->dev.kobj, "input");
    if (err < 0) {
...
#endif
#ifdef JD_EARPHONE_DETECT
    headset_notifier_unregister(&ts->earphone_notify);
#endif
#if defined(JD_USB_DETECT_CALLBACK)
