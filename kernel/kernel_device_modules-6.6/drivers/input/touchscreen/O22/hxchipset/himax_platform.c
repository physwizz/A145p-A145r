...
int himax_ts_unregister_interrupt(void)
{
    ...
    defined(HX_CONFIG_PM) || defined(HX_CONFIG_DRM_PANEL)||defined(HX_CONFIG_DRM_MTK)
        ...
        unsigned long event, void *data)
{
    ...
    int *blank;
    struct himax_ts_data *ts =
        container_of(self, struct hx_ts_data, fb_notif);
...
static void himax_notifier_register(struct work_struct *work)
{
    ...
    ts->hx_notif.notifier_call = hx_mtk_disp_notifier_callback;
	ret = mtk_disp_notifier_register("Touch", &ts->hx_notif);
...
static int __init himax_common_init(void)
{
    ...
    if (tp_detect_panel("hx83112")) {
        I("No need to load hx83112 touch driver\n");
    } else {
        I("load hx83112 touch driver\n");
