...
static const struct proc_ops cts_info_proc_fops = {
    .proc_read = cts_proc_getinfo_read,
};
...
#endif
{
    ...
    if (ret) {
        cts_err("Init drm notifier failed %d", ret);
...
#ifdef CONFIG_CTS_I2C_HOST
RET_REMOVE cts_driver_remove(struct i2c_client *client)
#else
RET_REMOVE  cts_driver_remove(struct spi_device *client)
#endif
{
    struct chipone_ts_data *cts_data;
    int ret = 0;
...
static int __init cts_driver_init(void)
{
    ...
    if (tp_detect_panel("nl9922c")) {
        cts_info("No need to load nl9922c touch driver\n");
    } else {
        cts_info("load nl9922c touch driver\n");
        spi_register_driver(&cts_spi_driver);
    }
