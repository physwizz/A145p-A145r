...
static int update_earphone_state(bool state);
...
/*hs14 code for AL6528ADEU-966 by liudi at 20221024 start*/
SHOW_STORE_PROTOTYPE(ovt_tcm, earphone_connected)
/*hs14 code for AL6528ADEU-966 by liudi at 20221024 end*/
...
/*hs14 code for AL6528ADEU-966 by liudi at 20221024 start*/
dynamic_config_sysfs(earphone_connected, DC_EARPHONE_CONNECTED)
/*hs14 code for AL6528ADEU-966 by liudi at 20221024 end*/
...
get_info:
    ...
    LOGI(tcm_hcd->pdev->dev.parent,"IC_NAME:%s TOUCH_VER:%02x\n",tcm_hcd->tp_module_name,tcm_hcd->app_info.customer_config_id[FW_VER] & 0xFF);
...
static int update_charger_state(bool state)
{
    ...
    if (tcm_hcd->charger_state) {
        retval = tcm_hcd->set_dynamic_config(tcm_hcd, DC_CHARGER_CONNECTED, 0x01);
    } else {
        retval = tcm_hcd->set_dynamic_config(tcm_hcd, DC_CHARGER_CONNECTED, 0x00);
    }

    if (retval < 0) {
        pr_err("%s Failed to set dynamic config\n",__func__);
    }
    pr_err("%s status is %d\n", __func__,tcm_hcd->charger_state);
    mutex_unlock(&tcm_hcd->extif_mutex);
    return retval;
}
...
static int tp_usb_check_notify_callback(struct notifier_block *this,unsigned long event,void *ptr)
{
    ...
    {
        g_tcm_hcd->charger_state = true;
    } else if (event == TP_USB_PLUGOUT_STATE) {
        g_tcm_hcd->charger_state = false;
    } else {
        LOGE(g_tcm_hcd->pdev->dev.parent,"%s():tpusb err\n",__func__);
        return -EINVAL;
    }
    if (g_tcm_hcd->in_suspend == false && g_tcm_hcd->fw_ready == true) {
        update_charger_state(g_tcm_hcd->charger_state);
    }
    pr_err("%s status is %lu\n", __func__,event);
    return 0;
}
/*hs14 code for SR-AL6528A-01-473 by hehaoran5 at 20221020 end*/
...
static int update_earphone_state(bool state){
    ...
    if (tcm_hcd->earphone_enable) {
        retval = tcm_hcd->set_dynamic_config(tcm_hcd, DC_EARPHONE_CONNECTED, 0x01);
    } else {
        retval = tcm_hcd->set_dynamic_config(tcm_hcd, DC_EARPHONE_CONNECTED, 0x00);
    }

    if (retval < 0) {
        LOGE(tcm_hcd->pdev->dev.parent,"%s Failed to set dynamic config\n",__func__);
    }
    pr_err("%s status is %d\n", __func__,tcm_hcd->earphone_enable);
    mutex_unlock(&tcm_hcd->extif_mutex);
    return retval;
}

static int tp_earphone_notifier_callback(struct notifier_block *this,unsigned long event,void *ptr)
{
    if (g_tcm_hcd == NULL) {
        return -EFAULT;
    }
    if (event == HEADSET_PLUGIN_STATE) {
        g_tcm_hcd->earphone_enable = 1;
    } else if (event == HEADSET_PLUGOUT_STATE) {
        g_tcm_hcd->earphone_enable = 0;
    } else {
        LOGE(g_tcm_hcd->pdev->dev.parent,"%s():earphone err\n",__func__);
        return -EINVAL;
    }
    if (g_tcm_hcd->in_suspend == false && g_tcm_hcd->fw_ready == true) {
        update_earphone_state(g_tcm_hcd->earphone_enable);
    }
    pr_err("%s status is %d\n", __func__,g_tcm_hcd->earphone_enable);
    return 0;
}
/*hs14 code for AL6528ADEU-966 by liudi at 20221024 end*/
...
void ovt_tcm_enable_gesture_wakeup(void)
{
    LOGE(g_tcm_hcd->pdev->dev.parent, "ovt_tcm_enable_gesture_wakeup\n");
    g_tcm_hcd->wakeup_gesture_enabled = true;
    g_smart_wakeup_flag = true;
    LOGE(g_tcm_hcd->pdev->dev.parent,
                "wakeup_gesture_enabled = %d",
                g_tcm_hcd->wakeup_gesture_enabled);
}
void ovt_tcm_disable_gesture_wakeup(void)
{
    LOGE(g_tcm_hcd->pdev->dev.parent, "ovt_tcm_enable_gesture_wakeup\n");
...
{
    printk("%s(): not ok ",__func__);
    return 0;
}
static void tp_feature_interface(void)
{
    struct ovt_tcm_hcd *tcm_hcd = g_tcm_hcd;
    struct lcm_info *gs_ovt_tcm_lcm_info = NULL;

    gs_ovt_tcm_lcm_info = kzalloc(sizeof(*gs_ovt_tcm_lcm_info), GFP_KERNEL);
    if (gs_ovt_tcm_lcm_info == NULL) {
        kfree(gs_ovt_tcm_lcm_info);
        LOGE(tcm_hcd->pdev->dev.parent,"gs_ovt_tcm_lcm_info kzalloc fail, and is NULL!");
        return;
    }

    gs_ovt_tcm_lcm_info->module_name = tcm_hcd->tp_module_name;
    gs_ovt_tcm_lcm_info->sec = tcm_hcd->sec;
    gs_ovt_tcm_lcm_info->fw_version = &(tcm_hcd->app_info.customer_config_id[FW_VER]);
    /*gesture wakeup*/
    gs_ovt_tcm_lcm_info->gesture_wakeup_enable = ovt_tcm_enable_gesture_wakeup;
    gs_ovt_tcm_lcm_info->gesture_wakeup_disable = ovt_tcm_disable_gesture_wakeup;
    /*ITO TEST*/
    gs_ovt_tcm_lcm_info->ito_test = td_ito_test;
    LOGE(tcm_hcd->pdev->dev.parent, "module_name=%s", gs_ovt_tcm_lcm_info->module_name);

    tp_common_init(gs_ovt_tcm_lcm_info);
}
...
static int ovt_tcm_drm_notifier_cb(struct notifier_block *nb,
        unsigned long action, void *data)
{
    int retval;
    int *evdata = (int *)data;
    struct ovt_tcm_hcd *tcm_hcd =
            container_of(nb, struct ovt_tcm_hcd, drm_notifier);
...
static int ovt_tcm_esd_recovery_notifier_cb(struct notifier_block *nb,
        unsigned long action, void *data)
{
    int retval = 0;

    struct ovt_tcm_hcd *tcm_hcd = container_of(nb, struct ovt_tcm_hcd, esd_recovery_notifier);

    if (action == FB_BLANK_POWERDOWN) {
        retval = ovt_tcm_suspend(&tcm_hcd->pdev->dev);
        tcm_hcd->fb_ready = 0;
    } else if (action == FB_BLANK_UNBLANK) {
        retval = ovt_tcm_resume(&tcm_hcd->pdev->dev);
        tcm_hcd->fb_ready++;
    }
    return 0;
...
static void ovt_tcm_compatible_panels(struct ovt_tcm_hcd *tcm_hcd)
{
    if (lcm_detect_panel("lcd_td4375_txd_jdi_mipi_fhd_video")) {
    ...
    } else if (lcm_detect_panel("lcd_td4375_cw_inx_mipi_fhd_video")) {
        printk("%s:panel= lcd_td4375_cw_inx_mipi_fhd_video\n",__func__);
        strcpy(tcm_hcd->tp_firmware_name,"td4375_cw_inx_firmware.img");
        strcpy(tcm_hcd->tp_module_name,"TD4375_CW_INX");
        strcpy(tcm_hcd->tp_limits_name,"td4375_cw_inx_mp_test.csv");
    }
}
...
static int ovt_tcm_probe(struct platform_device *pdev)
{
    ...
    ovt_tcm_compatible_panels(tcm_hcd);
...
/*hs14 code for SR-AL6528A-01-458 by hehaoran5 at 20220913 end*/
    ...
    LOGI(&pdev->dev,"%s(): start tp_feature_interface\n",__func__);
...
#elif CONFIG_DRM
   tcm_hcd->drm_notifier.notifier_call = ovt_tcm_drm_notifier_cb;
    retval = mtk_disp_notifier_register("Touch",&tcm_hcd->drm_notifier);
    if (retval < 0) {
        LOGE(tcm_hcd->pdev->dev.parent,
    ...
    /*hs14 code for SR-AL6528A-01-473 by hehaoran5 at 20221020 start*/
    tcm_hcd->tp_usb_notify.notifier_call = tp_usb_check_notify_callback;
    retval=register_usb_check_notifier(&tcm_hcd->tp_usb_notify);
    if (retval) {
        LOGE(&pdev->dev,"tp_usb_notify register fail\n");
    }
    /*hs14 code for SR-AL6528A-01-473 by hehaoran5 at 20221020 end*/
    /*hs14 code for AL6528ADEU-966 by liudi at 20221024 start*/
    tcm_hcd->earphone_notify.notifier_call = tp_earphone_notifier_callback;
    retval = headset_notifier_register(&tcm_hcd->earphone_notify);
    if (retval) {
        LOGE(&pdev->dev,"earphone_notify register fail\n");
    }
    /*hs14 code for AL6528ADEU-966 by liudi at 20221024 end*/
#endif
...
#elif CONFIG_DRM
    mtk_disp_notifier_unregister(&tcm_hcd->drm_notifier);
#endif
...
{
    int retval;
    if (tp_detect_panel("td4375")) {
        printk("%s:No need to load td4375 touch driver\n",__func__);
        return 0;
    }
    device_module_init();
