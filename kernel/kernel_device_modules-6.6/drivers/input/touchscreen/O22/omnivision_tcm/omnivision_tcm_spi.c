...
static int parse_dt(struct device *dev, struct ovt_tcm_board_data *bdata)
{
    ...
    if (prop && prop->length) {
        bdata->irq_gpio = of_get_named_gpio(np, "omnivision,irq-gpio", 0);
    } else {
        bdata->irq_gpio = -1;
    }
...
{
    printk("%s:ovt_tcm Init\n",__func__);
    if (tp_detect_panel("td4375")) {
        printk("%s:No need to load td4375 touch driver\n",__func__);
    } else {
        printk("%s:load td4375 touch driver\n",__func__);
        spi_register_driver(&ovt_tcm_spi_driver);
    }
