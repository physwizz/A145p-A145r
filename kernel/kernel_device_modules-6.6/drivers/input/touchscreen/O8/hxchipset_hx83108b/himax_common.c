...
static int hx_proc_selftest_seq_read(struct seq_file *s, void *v)
{
    ...
    if (chip_test_r_flag) {
#if defined(CONFIG_TOUCHSCREEN_HIMAX_INSPECT_O8)
        if (g_rslt_data) {
...
#elif defined(CONFIG_TOUCHSCREEN_HIMAX_INSPECT_O8) || defined(HX_PALM_REPORT)
...
static int i_get_FW(void)
{
    ...
    /*A06 code for SR-AL7160A-01-65 by wenghailong at 20240403 start*/
    ret = request_firmware(&hxfw, hx_s_ts->himax_nomalfw_rq_name, hx_s_ts->dev);
    I("%s: request file %s finished\n", __func__, hx_s_ts->himax_nomalfw_rq_name);
    /*A06 code for SR-AL7160A-01-65 by wenghailong at 20240403 end*/
    if (ret < 0) {
#if defined(HX_FIRMWARE_HEADER)
...
#if defined(DD_VSN_RECOV_EXCP)    
if (result == ERR_FW_EMPTY) {
        hx_s_ts->update_fw_fail = 0;
        hx_s_ts->update_fw_flag = 0;
        I("%s:fw is empty\n", __func__ );
    } else if ( result ){
        hx_s_ts->update_fw_fail++;
        hx_s_ts->update_fw_flag = 0;
        I("%s:himax  TP upgrade error, update_fw_fail = %d, update_fw_flag = %d\n", __func__, hx_s_ts->update_fw_fail, hx_s_ts->update_fw_flag);
    } else {
        hx_s_ts->update_fw_fail = 0;
        hx_s_ts->update_fw_flag++;
 ...
 */
    ...
    if ((hx_excp_event == length || hx_zero_event == length)
        ...
        if (shaking_ret == HX_EXCP_EVENT || 
            ...
            himax_excp_hw_reset();
#if defined(DD_VSN_RECOV_EXCP)
            if (hx_s_ts->update_fw_fail == 3) {
                hx_s_ts->update_fw_fail = 0;
                hx_s_ts->vsn_flag = 1;
                himax_mcu_vsn_recovery_flag = 1;
            }

            if (hx_s_ts->vsn_flag == 1) {
                if (hx_s_ts->update_fw_flag == 4) {
                    hx_s_ts->update_fw_flag = 0;
...
int himax_report_data(struct himax_ts_data *ts, int ts_path, int ts_status)
{
    ...
    if (ts_path == HX_REPORT_COORD || ts_path == HX_REPORT_COORD_RAWDATA) {
        ...
        /*a06 code for AL7160A-5 by wenghailong at 20240320 start*/
        if (ts->hx_point_num != 0) {
            if (get_tp_enable()) {
        ...
        /*a06 code for AL7160A-5 by wenghailong at 20240320 end*/
...
#if defined(CONFIG_TOUCHSCREEN_HIMAX_IC_HX83108A_O8)
    if (_hx83108a_init()) {
        ret = NO_ERR;
        goto END;
    }
...
{
    if (hx_s_ts->suspended == false) {
        I("enter headset_workqueue\n");
        hx_s_core_fp._set_headphone_en(hx_s_ts->hp_en, hx_s_ts->suspended);
    } else {
...
/*a06 code for SR-AL7160A-01-111  by suyurui at 20240411 start*/
static void himax_usb_workqueue(struct work_struct *work)
{
    if (hx_s_ts->suspended == false) {
        I("enter usb_workqueue\n");
        hx_s_core_fp._usb_detect_set(hx_s_ts->cable_config);
    } else {
        I("%s:is suspended,do nothing", __func__);
    }
...
{
    if (hx_s_ts->cable_config) {
        if (event == TP_USB_PLUGIN_STATE) {
            hx_s_ts->cable_config[1] = 0x01;
        } else if (event == TP_USB_PLUGOUT_STATE) {
            hx_s_ts->cable_config[1] = 0x00;
        } else {
            return -EINVAL;
        }
    }
    schedule_work(&hx_s_ts->tp_usb_work_queue);
    return 0;
}
/*a06 code for SR-AL7160A-01-111  by suyurui at 20240411 end*/
...
static int himax_get_tp_module(void)
{
    ...
    if (tp_choose_panel("hx83108b_xx_hkc")) {
        fw_num = MODEL_XX_HKC;
    } else if (tp_choose_panel("hx83108b_ls_hkc")) {
        fw_num = MODEL_LS_HKC;
    } else if (tp_choose_panel("hx83108b_ls_inx")) {
        fw_num = MODEL_LS_INX;
    } else {
...
static int himax_get_tp_module(void)
{
    ...
    if (NULL != strstr(panel_name, "hx83108b_xx_hkc")) {
        fw_num = MODEL_XX_HKC;
    } else if (NULL != strstr(panel_name, "hx83108b_ls_hkc")) {
        fw_num = MODEL_LS_HKC;
    } else if (NULL != strstr(panel_name, "hx83108b_ls_inx")) {
        fw_num = MODEL_LS_INX;
    } else {
        fw_num = MODEL_DEFAULT;
    }
...
static void himax_update_module_info(void)
{
    ...
    {
    case MODEL_XX_HKC:
            strcpy(ts->module_name,"hx83108b_xx_hkc");
            strcpy(ts->himax_nomalfw_rq_name, "hx83108b_xx_hkc_firmware.bin");
            strcpy(ts->himax_mpfw_rq_name, "hx83108b_xx_hkc_mpfw.bin");
            strcpy(ts->himax_csv_name, "hx83108b_xx_hkc_mp_test.csv");
            break;
    case MODEL_LS_HKC:
            strcpy(ts->module_name,"hx83108b_ls_hkc");
            strcpy(ts->himax_nomalfw_rq_name, "hx83108b_ls_hkc_firmware.bin");
            strcpy(ts->himax_mpfw_rq_name, "hx83108b_ls_hkc_mpfw.bin");
            strcpy(ts->himax_csv_name, "hx83108b_ls_hkc_mp_test.csv");
            break;
    case MODEL_LS_INX:
            strcpy(ts->module_name,"hx83108b_ls_inx");
            strcpy(ts->himax_nomalfw_rq_name, "hx83108b_ls_inx_firmware.bin");
            strcpy(ts->himax_mpfw_rq_name, "hx83108b_ls_inx_mpfw.bin");
            strcpy(ts->himax_csv_name, "hx83108b_ls_inx_mp_test.csv");
            break;
    default:
...
static void tp_feature_interface(void)
{
    ...
    if (gs_hx_lcm_info == NULL) {
        ...
        E("gs_hx_lcm_info kzalloc fail, and is NULL!");
        return;
    }

    gs_hx_lcm_info->module_name = ts->module_name;
    gs_hx_lcm_info->sec = ts->sec;
    gs_hx_lcm_info->fw_version = &(hx_s_ic_data->vendor_touch_cfg_ver);
    /*gesture wakeup*/
    ...
    /*ITO TEST*/
    gs_hx_lcm_info->ito_test = hx_s_core_fp.fp_chip_self_test_for_tp;
 ...
 */
...
#endif
    /*a06 code for AL7160A-76 by suyurui at 20240417 start*/
    ts->cable_config = pdata->cable_config;
    /*a06 code for AL7160A-76 by suyurui at 20240417 end*/
...
#if defined(CONFIG_TOUCHSCREEN_HIMAX_DEBUG_O8)
    if (himax_debug_init()) {
    ...
    if (ret) {
        ...
        goto err_headset_notify_failed;
    }
    /*a06 code for SR-AL7160A-01-111  by suyurui at 20240416 end*/
    ...
    if (ret) {
        ...
        goto err_usb_check_notify_failed;
    }
    /*a06 code for SR-AL7160A-01-111  by suyurui at 20240411 end*/
