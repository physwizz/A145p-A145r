...
/*A06 code for AL7160AV-12 by guyuming at 20240830 start*/
...
/*A06 code for AL7160AV-12 by guyuming at 20240830 end*/
...
static void gcore_usb_earphone_notify_workqueue(struct work_struct *work)
{
    ...
    if (gdev == NULL) {
        GTP_ERROR("%s gdev is NULL", __func__);
        return;
    }
...
void gcore_deinit(struct gcore_dev *gdev)
{
    ...
    if (disp_notify_reg_flag) {
        if (mtk_disp_notifier_unregister(&gdev->drm_notifier)) {
            GTP_ERROR("Error occurred when unregister disp_notifier");
        } else {
...
void gcore_enable_gesture_wakeup(void)
{
    ...
    GTP_DEBUG("g_smart_wakeup_flag = %d", g_smart_wakeup_flag);
}
...
static int gcore_get_tp_module(void)
{
    ...
    /*A06 code for AL7160AV-12 by guyuming at 20240830 start*/
    if (tp_choose_panel("gc7272_xx_hsd")) {
        fw_num = MODEL_XX_HSD;
    } else if (tp_choose_panel("gc7272_ld_ctc")) {
        fw_num = MODEL_LD_CTC;
    } else if (tp_choose_panel("gc7272_xx_boe")) {
        fw_num = MODEL_XX_BOE;
    } else {
...
static void tp_feature_interface(void)
{
    ...
    if (gs_gcore_lcm_info == NULL) {
        ...
        GTP_ERROR("gs_gcore_lcm_info kzalloc fail, and is NULL!");
        return;
    }
...
#if IS_ENABLED(CONFIG_DEVICE_MODULES_DRM_MEDIATEK)
    GTP_DEBUG("Init notifier_mtk_drm struct");
	gdev->drm_notifier.notifier_call = gcore_ts_drm_notifier_callback;
	if (mtk_disp_notifier_register("Touch", &gdev->drm_notifier)) {
        GTP_ERROR("[DRM]Failed to register disp notifier client");	
    } else {
		disp_notify_reg_flag = true;
        GTP_DEBUG("[DRM]Success to register disp notifier client");	
    }
#endif//CONFIG_DEVICE_MODULES_DRM_MEDIATEK
...
#endif
#endif
/*A06 code for AL7160AV-36 by huangyin at 20241023 start*/
    gcore_modify_fw_event_cmd(DRIVER_REGISTER_START);
/*A06 code for AL7160AV-36 by huangyin at 20241023 end*/
