...
void set_tp_enable(bool enabled)
{
    g_tp_is_enabled = enabled;
}
EXPORT_SYMBOL(set_tp_enable);
...
enum boot_mode_t tp_get_boot_mode(void) {
	...
	tag = (struct atag_bootmode *)of_get_property(np_chosen, "atag,boot", NULL);
	if (!tag) {
...
EXPORT_SYMBOL(headset_notifier_call_chain);
/*--------------ear notifier end------------*/

/*---------------usb notifier start-------------*/
static BLOCKING_NOTIFIER_HEAD(usb_check_chain);

int register_usb_check_notifier(struct notifier_block *nb)
{
    return blocking_notifier_chain_register(&usb_check_chain,nb);
}
EXPORT_SYMBOL(register_usb_check_notifier);

int unregister_usb_check_notifier(struct notifier_block *nb)
{
    return blocking_notifier_chain_unregister(&usb_check_chain,nb);
}
EXPORT_SYMBOL(unregister_usb_check_notifier);

int usb_check_notifier_call_chain(unsigned long val,void *v)
{
    return blocking_notifier_call_chain(&usb_check_chain,val,v);
}
