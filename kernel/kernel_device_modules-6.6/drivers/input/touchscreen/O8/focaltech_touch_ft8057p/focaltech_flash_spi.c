...
struct upgrade_module module_list[] = {
    ...
    {FTS_MODULE3_ID, FTS_MODULE3_NAME, fw_file, sizeof(fw_file)},
};
...
static int fts_fw_resume(bool need_reset, enum FW_TYPE fw_type)
{
    ...
    if (FTS_FW_REQUEST_SUPPORT) {
        ...
        if (fts_data->fw_name) {
            snprintf(fwname, FILE_NAME_LENGTH, "%s",fts_data->fw_name);
        } else {
...
_release_firmware:
    ...
    /*A06 code for SR-AL7160A-01-215 by wenghailong at 20240402 start*/
    fts_read_reg(FTS_REG_FW_VER, &(fts_data->firmware_ver));
    /*A06 code for SR-AL7160A-01-215 by wenghailong at 20240402 end*/
