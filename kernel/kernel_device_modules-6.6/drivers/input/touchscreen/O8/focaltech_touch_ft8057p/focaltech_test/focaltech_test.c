...
{
    static char time_data_buf[TIME_BUFF_LEN] = {0};
...
static int fts_test_save_test_data(char *file_name, char *data_buf, int len)
{
    ...
    FTS_INFO("module_name = %s, fts_ftest->result = %d",  fts_data->module_name, fts_ftest->result);
    switch (fts_ftest->result) {
        case TEST_RESULT_PASS:
            snprintf(filepath, FILE_NAME_LENGTH, "%sPASS_%s_%s_%s", FTS_INI_FILE_PATH,
                get_date_time_str(), fts_data->module_name, file_name);
            break;
        case TEST_RESULT_FAIL:
            snprintf(filepath, FILE_NAME_LENGTH, "%sFAIL_%s_%s_%s", FTS_INI_FILE_PATH,
                get_date_time_str(), fts_data->module_name, file_name);
            break;
        default:
            FTS_INFO("TP test no result, fts_ftest->result = %d\n", fts_ftest->result);
            break;
    }
...
int fts_ito_test(void)
{
    ...
    atomic_set(&fts_data->ito_testing, 1);//start
    memset(fwname, 0, sizeof(fwname));
    FTS_INFO("fts_data->ito_file = %s", fts_data->ito_file);
    if (fts_data->ito_file) {
        snprintf(fwname, FILE_NAME_LENGTH, "%s",fts_data->ito_file);
    }

    FTS_INFO("ito file name = %s", fts_data->ito_file);
    FTS_TEST_DBG("fwname:%s.", fwname);

    mutex_lock(&input_dev->mutex);
    fts_irq_disable();
    #if FTS_ESDCHECK_EN
    fts_esdcheck_switch(fts_data, DISABLE);
    #endif
    ret = fts_enter_test_environment(1);
    if (ret < 0) {
        FTS_ERROR("enter test environment fail");
    ...
    #if FTS_ESDCHECK_EN
    fts_esdcheck_switch(fts_data, ENABLE);
    #endif
    fts_irq_enable();
    mutex_unlock(&input_dev->mutex);
    atomic_set(&fts_data->ito_testing, 0);//end
    FTS_INFO("ito result = %d", result);
    FTS_INFO("----");
    return result;
    #else
    FTS_INFO("ito test is not enabled");
    return -EINVAL;
    #endif
}
/*A06 code for SR-AL7160A-01-88 by wenghailong at 20240404 end*/
