...
static bool disp_notify_reg_flag = false;
#endif//CONFIG_DEVICE_MODULES_DRM_MEDIATEK
...
static int fts_get_ic_information(struct fts_ts_data *ts_data)
{
    ...
    /*A06 code for AL7160A-1123 by wenghailong at 20240514 start*/
    for (cnt = 0; cnt < 4; cnt++) {
        if (cnt == 3) {
            FTS_DEBUG("get ic informaton, retry:%d", cnt);
            mdelay(200);
        }
    ...
    /*A06 code for AL7160A-1123 by wenghailong at 20240514 end*/
    ...
    for (i = 0; i < ts_data->touch_event_num; i++) {
        ...
        if (EVENT_DOWN(events[i].flag)) {
            ...
            if ((ts_data->log_level >= 2) ||
                ...
                /*A06 code for AL7160AV-34 by huangyin at 20241016 start*/
                #if defined(HQ_FACTORY_BUILD)
                FTS_DEBUG("[B]P%d(%d, %d)[p:%d,tm:%d] DOWN!",
                          events[i].id, events[i].x, events[i].y,
                          events[i].p, events[i].area);
                #endif//HQ_FACTORY_BUILD
                /*A06 code for AL7160AV-34 by huangyin at 20241016 end*/
            }
        } else {
            input_mt_slot(input_dev, events[i].id);
            input_mt_report_slot_state(input_dev, MT_TOOL_FINGER, false);
...
static int fts_read_parse_touchdata(struct fts_ts_data *ts_data, u8 *touch_buf)
{
    ...
    /*read touch data*/
    /*A06 code for SR-AL7160A-01-94  by yuli at 20240409 start*/
    if (get_tp_enable()) {
    ...
    /*A06 code for SR-AL7160A-01-94  by yuli at 20240409 end*/
#if FTS_SAMSUNG_SCREEN_SHOT_EN
    ...
    /*A06 code for SR-AL7160A-01-94  by yuli at 20240409 start*/
    if (!get_tp_enable()) {
        FTS_INFO("get_tp_enable() is false, dont report point");
        return TOUCH_IGNORE;
    }
    /*A06 code for SR-AL7160A-01-94  by yuli at 20240409 end*/
...
static int fts_ts_resume(struct device *dev)
{
    ...
    FTS_INFO("core FW version = 0x%02x", fts_data->firmware_ver);
...
#if IS_ENABLED(CONFIG_DEVICE_MODULES_DRM_MEDIATEK)
	if (mtk_disp_notifier_register("Touch", &ts_data->fb_notif)) {
		FTS_ERROR("Failed to register disp notifier client");
    } else {
        disp_notify_reg_flag = true;
        FTS_INFO("Success to register mtk_disp_notifier_register");
    }
...
static int fts_notifier_callback_exit(struct fts_ts_data *ts_data)
{
    ...
    if (disp_notify_reg_flag) {
        if (mtk_disp_notifier_unregister(&ts_data->fb_notif)) {
            FTS_INFO("Error occurred when unregister disp_notifier");
        } else {
            disp_notify_reg_flag = false;
            FTS_INFO("success unregister disp_notifier");
        }
...
/*A06 code for SR-AL7160A-01-77 by wenghailong at 20240403 start*/
fts_upgrade_module fts_module_list[] = {
    {
        .panel_name = "ft8057p_xx_ctc",
        .fw_num = MODEL_XX_CTC,
    },
    {
        .panel_name = "ft8057p_txd_boe",
...
static int fts_get_tp_module(void)
{
    ...
    for (i = 0; i < ARRAY_SIZE(fts_module_list); i++) {
        if (tp_choose_panel(fts_module_list[i].panel_name)) {
...
static int fts_get_tp_module(void)
{
    ...
    active_panel_name = tp_choose_panel();
...
static void fts_update_module_info(void)
{
    ...
    module = fts_get_tp_module();

    switch (module)
    {
    case MODEL_XX_CTC:
            fts_data->fw_name = "focaltech_ts_fw_ctc.bin";
            fts_data->ito_file = "focaltech_ts_ito_ctc.ini";
            fts_data->module_name = "ft8057p_xx_ctc";
            break;
    case MODEL_TXD_BOE:
            fts_data->fw_name = "focaltech_ts_fw_boe.bin";
            fts_data->ito_file = "focaltech_ts_ito_boe.ini";
            fts_data->module_name = "ft8057p_txd_boe";
            break;
    default:
...
/*A06 code for SR-AL7160A-01-89 by yuli at 20240408 start*/
...
void fts_gesture_wakeup_disable(void)
{
    ...
    g_smart_wakeup_flag = DISABLE;
    mutex_unlock(&fts_data->input_dev->mutex);
}
/*A06 code for SR-AL7160A-01-89 by yuli at 20240408 end*/
...
static void tp_feature_interface(void)
{
    ...
    if (gs_fts_lcm_info == NULL) {
        ...
        FTS_ERROR("gs_fts_lcm_info kzalloc fail, and is NULL!");
        return;
    }

    gs_fts_lcm_info->module_name = ts_data->module_name;
    gs_fts_lcm_info->sec = ts_data->sec;
    gs_fts_lcm_info->fw_version = &(ts_data->firmware_ver);
    /*gesture wakeup*/
    /*A06 code for SR-AL7160A-01-89 by yuli at 20240408 start*/
    ...
    /*A06 code for SR-AL7160A-01-89 by yuli at 20240408 end*/
    /*ITO TEST*/
    /*A06 code for SR-AL7160A-01-88 by wenghailong at 20240404 start*/
    gs_fts_lcm_info->ito_test = fts_ito_test;
    /*A06 code for SR-AL7160A-01-88 by wenghailong at 20240404 end*/
    FTS_INFO("module_name=%s", gs_fts_lcm_info->module_name);

    tp_common_init(gs_fts_lcm_info);

}
/*A06 code for SR-AL7160A-01-77 by wenghailong at 20240403 end*/
...
int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
    ...
    /*A06 code for SR-AL7160A-01-88 by wenghailong at 20240404 start*/
    atomic_set(&fts_data->ito_testing, 0);
