...
__attribute__((unused)) static int sc8960x_soft_enable_hvdcp(struct sc8960x_chip *sc)
{
    ...
    /*A14_V code for SR-AL6528V-01-129 by chenyulin at 20241009 start*/
    if (sc->sdp_detect == false)
    {
        dev_err(sc->dev, "%s: sdp_detect = %d\n", __func__, sc->sdp_detect);
    }
    /*A14_V code for SR-AL6528V-01-129 by chenyulin at 20241009 end*/
...
static int __sc8960x_write_reg(struct sc8960x_chip *sc, int reg, u8 val)
{
    s32 ret;
...
static int sc8960x_write_byte(struct sc8960x_chip *sc, u8 reg, u8 data)
{
    int ret;
...
static int __sc8960x_read_reg(struct sc8960x_chip *sc, u8 reg, u8 *data)
{
    s32 ret;
    /* hs14 code for SR-AL6528A-01-787 by gaozhengwei at 2022/12/06 start */
    int i;
...
static int sc8960x_read_byte(struct sc8960x_chip *sc, u8 reg, u8 *data)
{
    int ret;
...
static int sc8960x_set_wa_key(struct sc8960x_chip *sc)
{
...
static int sc8960x_enable_bc12(struct sc8960x_chip *sc, bool en)
{
    int ret;
    u8 reg_val;
...
static int sc8960x_chg_set_usbsw(struct sc8960x_chip *sc,
		enum sc8960x_usbsw usbsw)
{
    struct phy *phy = NULL;
    int ret = 0; 
...
static int sc8960x_charger_probe(struct i2c_client *client)
{
    ...
    /*Tab A9 code for SR-AX6739A-01-489 by wenyaqi at 20230504 start*/
    /* set for chg_info */
    gxy_bat_set_chginfo(GXY_BAT_CHG_INFO_SC8960X);
