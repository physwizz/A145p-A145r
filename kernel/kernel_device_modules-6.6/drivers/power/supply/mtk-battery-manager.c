...
static enum power_supply_property battery_props[] = {
	...
	/*A06_V code for SR-AL7160V-01-159 by xiongxiaoliang at 20240924 start*/
	POWER_SUPPLY_PROP_VOLTAGE_OCV,
	/*A06_V code for SR-AL7160V-01-159 by xiongxiaoliang at 20240924 end*/
};
...
static int bm_update_psy_property(struct mtk_battery *gm, enum bm_psy_prop prop)
{
	...
	/*A06_V code for SR-AL7160V-01-159 by xiongxiaoliang at 20240924 start*/
	case VOLTAGE_OCV:
		voltage_ocv = gauge_get_int_property(gm, GAUGE_PROP_BOOT_ZCV);
		ret_val = voltage_ocv;
		break;
	/*A06_V code for SR-AL7160V-01-159 by xiongxiaoliang at 20240924 end*/
	default:
...
/*A06_V code for SR-AL7160V-01-161 by xiongxiaoliang at 20240919 end*/
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		/*A06_V code for SR-AL7160V-01-153 by xiongxiaoliang at 20240913 start*/
		if ((bm->bootmode == 8) &&
			(val->intval == POWER_SUPPLY_STATUS_NOT_CHARGING)) {
			val->intval = POWER_SUPPLY_STATUS_CHARGING;
		}
		/*A06_V code for SR-AL7160V-01-153 by xiongxiaoliang at 20240913 end*/
		break;
...
/*A06_V code for SR-AL7160V-01-161 by xiongxiaoliang at 20240919 end*/
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE:
		if (IS_ERR_OR_NULL(bs_data->chg_psy)) {
			...
			bs_data->chg_psy = power_supply_get_by_name("charger");
			#else
	...
	case POWER_SUPPLY_PROP_VOLTAGE_OCV:
		...
		if (bm->gm1 != NULL)
			if(!bm->gm1->bat_plug_out) {
				volt_ocv += bm_update_psy_property(bm->gm1, VOLTAGE_OCV);
				count += 1;
			}
		if (bm->gm2 != NULL)
			if(!bm->gm2->bat_plug_out) {
				volt_ocv += bm_update_psy_property(bm->gm2, VOLTAGE_OCV);
				count += 1;
			}
		if (count != 0)
			val->intval = volt_ocv / count * 100;
		ret = 0;
		break;
	/*A06_V code for SR-AL7160V-01-159 by xiongxiaoliang at 20240924 end*/
	default:
...
static int mtk_bm_probe(struct platform_device *pdev)
{
	...
	/*A06_V code for SR-AL7160V-01-132 by xiongxiaoliang at 20240820 start*/
	#if IS_ENABLED(CONFIG_HQ_PROJECT_O8) || IS_ENABLED(CONFIG_HQ_PROJECT_O22)
	bm->bs_data.chg_psy = power_supply_get_by_name("charger");
	#else
