...
/*a06 code for SR-AL7160V-01-62 by wenghailong at 20240826 start*/
#if defined(CONFIG_HQ_PROJECT_O8) || defined(CONFIG_HQ_PROJECT_O22)
#include "touch_feature.h"
#endif//CONFIG_HQ_PROJECT_O8
/*a06 code for SR-AL7160V-01-62 by wenghailong at 20240826 end*/
	...
	bool en)
{
	...
	if (info->algo.enable_charging != NULL){
	 #if IS_ENABLED(CONFIG_HQ_PROJECT_O8)
		if(!en) {
			g_cp_charging_lmt.cp_fac_stop_chg = 1;
		} else {
			g_cp_charging_lmt.cp_fac_stop_chg = 0;
		}
	#endif
...
int mtk_charger_set_shipmode(struct mtk_charger *info, bool enbale)
{
	...
	if (!ret) {
		chr_err("%s Set shipmode Fail!\n", __func__);
	}
	return ret;
}

int mtk_charger_get_shipmode(struct mtk_charger *info)
{
	int en = 0;
	struct charger_device *chg_dev = NULL;
				...
				struct device *dev)
{
	...
	if (of_property_read_u32(np, "vbus_sw_ovp_voltage", &val) >= 0)
		...
		chr_err("vbus_sw_ovp use default V_CHARGER_MAX:%d\n", V_CHARGER_MAX);
	...
	else if (of_property_read_u32(np, "jeita-temp-below-t0-cv", &val) >= 0)
		info->data.jeita_temp_below_t0_cv = val;
	else {
	...
	info->data.ap_temp_below_t1_cp_cc = AP_TEMP_BELOW_T1_CP_CC;
	info->data.ap_temp_t1_lcmon_cp_cc = AP_TEMP_T1_LCMON_CP_CC;
	info->data.ap_temp_t2_lcmon_cp_cc = AP_TEMP_T2_LCMON_CP_CC;
	info->data.ap_temp_t3_lcmon_cp_cc = AP_TEMP_T3_LCMON_CP_CC;
	info->data.ap_temp_normal_lcmon_cp_cc = AP_TEMP_NORMAL_LCMON_CP_CC;
	...
	info->data.ap_temp_t1_cp_thres_lcmon = AP_TEMP_T1_CP_THRES_LCMON;
	info->data.ap_temp_t2_cp_thres_lcmon = AP_TEMP_T2_CP_THRES_LCMON;
	info->data.ap_temp_t3_cp_thres_lcmon = AP_TEMP_T3_CP_THRES_LCMON;
	/*A06_V code for SR-AL7160V-01-163 by xiongxiaoliang at 20240912 start*/
	...
	rc = read_range_data_from_node(np, "gxy-cv-ranges",
		info->data.batt_cv_data, MAX_CYCLE_COUNT, info->data.battery_cv);
	if (rc < 0) {
		pr_err("Read gxy-cv-ranges failed from dtsi, rc=%d, gxy_batt_aging_enable=%d\n",
			rc, info->data.gxy_batt_aging_enable);
...
#if IS_ENABLED(CONFIG_HQ_PROJECT_O8)
/*A06_V code for SR-AL7160V-01-201 by xiongxiaoliang at 20240911 start*/
static void lcm_sts_switch(bool onoff);
#define AP_THERMAL_DEFAULT 25
static int get_ap_hw_temp(void)
{
	struct thermal_zone_device *thermal_dev = NULL;
	int temperature = 0;
	int ret = 0;

	thermal_dev = thermal_zone_get_zone_by_name("mtktsAP");
	if (IS_ERR(thermal_dev)) {
		chr_err("%s: Unable to get thermal zone for mtktsAP\n", __func__);
		goto AP_THERM_FAIL;
	}

	ret = thermal_zone_get_temp(thermal_dev, &temperature);
	if (ret) {
		chr_err("%s: Failed to get thermal zone temperature", __func__);
		goto AP_THERM_FAIL;
	}

	temperature /= 1000;
	chr_err("%s: get ap_thermal temperature = %d", __func__, temperature);
	return temperature;

AP_THERM_FAIL:
	return AP_THERMAL_DEFAULT;
}

void ap_thermal_machine_to_cp(struct mtk_charger *info)
{
	int ap_temp = 0;
	...
	g_cp_charging_lmt.cp_thermal_stop_chg = 0;
	...
	//Used for dynamic debugging
	if (info->ap_temp != 0xffff) {
		ap_temp = info->ap_temp;
	} else {
		ap_temp = get_ap_hw_temp();
	}
	...
	if (!info->lcm_sts) {
		...
		if (cp_lcmoff_data->sm == TEMP_ABOVE_T4) {
			...
			cp_lcmoff_data->cc = info->data.ap_temp_below_t1_cp_cc;
		}

		g_cp_charging_lmt.cp_therm_cc = cp_lcmoff_data->cc;
		...
		if (ap_temp >= info->data.ap_temp_t3_cp_thres_lcmon) {
			cp_lcmon_data->sm = TEMP_T3_TO_T4;
			chr_err("[AP_THERMAL_CP] LCDON AP T3 Over Temperature(%d) !!\n",
				info->data.ap_temp_t3_cp_thres_lcmon);
		} else if (ap_temp >= info->data.ap_temp_t2_cp_thres_lcmon) {
			cp_lcmon_data->sm = TEMP_T2_TO_T3;
			chr_err("[AP_THERMAL_CP] LCDON AP T2 Over Temperature(%d) !!\n",
				info->data.ap_temp_t2_cp_thres_lcmon);
		} else if (ap_temp >= info->data.ap_temp_t1_cp_thres_lcmon) {
			cp_lcmon_data->sm = TEMP_T1_TO_T2;
			chr_err("[AP_THERMAL_CP] LCDON AP T1 Over Temperature(%d) !!\n",
				info->data.ap_temp_t1_cp_thres_lcmon);
		}  else if (ap_temp < info->data.ap_temp_t1_cp_thres_lcmon) {
			cp_lcmon_data->sm = TEMP_T0_TO_T1;
			chr_err("[AP_THERMAL_CP] LCDON AP below Temperature(%d) !!\n",
				info->data.ap_temp_t1_cp_thres_lcmon);
		}

		if (cp_lcmon_data->sm == TEMP_T3_TO_T4) {
			cp_lcmon_data->cc = info->data.ap_temp_t3_lcmon_cp_cc;
			g_cp_charging_lmt.cp_thermal_stop_chg = 1;
		} else if (cp_lcmon_data->sm == TEMP_T2_TO_T3) {
			cp_lcmon_data->cc = info->data.ap_temp_t2_lcmon_cp_cc;
			g_cp_charging_lmt.cp_thermal_stop_chg = 1;
		} else if (cp_lcmon_data->sm == TEMP_T1_TO_T2) {
			cp_lcmon_data->cc = info->data.ap_temp_t1_lcmon_cp_cc;
			g_cp_charging_lmt.cp_thermal_stop_chg = 1;
		} else if (cp_lcmon_data->sm == TEMP_T0_TO_T1) {
			cp_lcmon_data->cc = info->data.ap_temp_normal_lcmon_cp_cc;
		}

		g_cp_charging_lmt.cp_therm_cc = cp_lcmon_data->cc;
...
/*A06_V code for SR-AL7160V-01-201 by xiongxiaoliang at 20240911 end*/
void ap_jeita_to_cp(int sm)
{
	int ap_jeita_cc = 0;
	g_cp_charging_lmt.cp_jeita_stop_chg = 0;

	switch (sm) {
	case TEMP_ABOVE_T4:
		ap_jeita_cc = JEITA_TEMP_ABOV_T4_CP_CC;
		g_cp_charging_lmt.cp_jeita_stop_chg = 1;
		break;
	case TEMP_T3_TO_T4:
		ap_jeita_cc = JEITA_TEMP_T3_TO_T4_CP_CC;
		g_cp_charging_lmt.cp_jeita_stop_chg = 0;
		break;
	case TEMP_T2_TO_T3:
		ap_jeita_cc = JEITA_TEMP_T2_TO_T3_CP_CC;
		g_cp_charging_lmt.cp_jeita_stop_chg = 0;
		break;
	case TEMP_T1_TO_T2:
		ap_jeita_cc = JEITA_TEMP_T1_TO_T2_CP_CC;
		g_cp_charging_lmt.cp_jeita_stop_chg = 1;
		break;
	case TEMP_T0_TO_T1:
		ap_jeita_cc = JEITA_TEMP_T0_TO_T1_CP_CC;
		g_cp_charging_lmt.cp_jeita_stop_chg = 1;
		break;
	case TEMP_BELOW_T0:
		ap_jeita_cc = JEITA_TEMP_BELOW_T0_CP_CC;
		g_cp_charging_lmt.cp_jeita_stop_chg = 1;
		break;
	default:
		break;
	...
	g_cp_charging_lmt.cp_jeita_cc = ap_jeita_cc;
	chr_err("[SW_JEITA_CP] cp_jeita_cc:%d ap_jeita_cc:%d\n",
		g_cp_charging_lmt.cp_jeita_cc, ap_jeita_cc);
}
#endif
/*A06_V code for SR-AL7160V-01-143 by xiongxiaoliang at 20240903 end*/
...
void do_sw_jeita_state_machine(struct mtk_charger *info)
{
	...
	#if IS_ENABLED(CONFIG_HQ_PROJECT_O8)
	ap_jeita_to_cp(sw_jeita->sm);
	g_cp_charging_lmt.cp_jeita_cv = sw_jeita->cv;
	chr_err("[SW_JEITA_CP]cp_jeita_cv===:%d\n",
		g_cp_charging_lmt.cp_jeita_cv);
	#endif
	/*A06_V code for SR-AL7160V-01-143 by xiongxiaoliang at 20240903 end*/
...
int gxy_get_prop_batt_current_event(struct mtk_charger *info)
{
	...
	chr_err("%s batt_current_event: %d, battery_temperature: %d\n", __func__, gxy_batt_current_event, battery_temperature);
...
/*A14_V code for  SR-AL6528V-01-118  by chenyulin at 20241021 start*/
#if IS_ENABLED(CONFIG_HQ_PROJECT_O22)
extern int aw_retry_src_caps(int slate_mode);
#endif
/*A14_V code for  SR-AL6528V-01-118  by chenyulin at 20241021 end*/
static void gxy_set_slate_mode(struct mtk_charger *info, int slate_mode)
{
	switch (info->gxy_tcpc_info) {
		...
		case GXY_BAT_TCPC_INFO_UPM7610:
		case GXY_BAT_TCPC_INFO_WUSB3812C:
		case GXY_BAT_TCPC_INFO_UPM6922P:
		/*A06_V code for SR-AL6528V-01-41 by shanxinkai at 20241008 start*/
		case GXY_BAT_TCPC_INFO_HUSB311:
		case GXY_BAT_TCPC_INFO_ET7304:
		/*A06_V code for SR-AL6528V-01-41 by shanxinkai at 20241008 end*/
			gxy_send_src_caps(slate_mode);
			break;
		/*A14_V code for  SR-AL6528V-01-118  by chenyulin at 20241021 start*/
		#if IS_ENABLED(CONFIG_HQ_PROJECT_O22)
		case GXY_BAT_TCPC_INFO_FUSB302B:
		...
		default:
			chr_err("%s: don't find tcpc_info slate_mode\n", __func__);
			break;
	}
...
void gxy_charger_init(struct mtk_charger *info)
{
	...
	info->gxy_cint.get_ship_mode = mtk_charger_get_shipmode;
	/*A06_V code for SR-AL7160V-01-164 by yexuedong at 20240910 end*/
	/*A06_V code for SR-AL7160V-01-161 by xiongxiaoliang at 20240919 start*/
...
static void charger_check_status(struct mtk_charger *info)
{
	...
	/*A14 code for SR-AL6528V-01-42 by shanxinkai at 20240923 start*/
	#if IS_ENABLED(CONFIG_HQ_PROJECT_O8) || IS_ENABLED(CONFIG_HQ_PROJECT_O22)
	/*A14 code for SR-AL6528V-01-42 by shanxinkai at 20240923 end*/
	...
	if (info->gxy_discharge_store_mode) {
		...
		chr_err("store mode for port charging = %d, chgen = %d\n",
			port_charging, chg_dev_port_chgen);
	} else {
		info->gxy_discharge_store_mode_flag = false;
	}
	#endif
...
stop_charging:
	...
	if ((s_aconline_old != info->ac_online) || (s_usbonline_old != info->usb_online)) {
		...
		chr_err("%s wire online is different\n", __func__);
		power_supply_changed(info->psy1);
	}
	#endif
	/*A06_V code for SR-AL7160V-01-132 by xiongxiaoliang at 20240820 end*/
...
static bool charger_init_algo(struct mtk_charger *info)
{
	...
	if (alg == NULL) {
		...
		/*A06_V code for P250221-05220 by shanxinkai at 20250226 start*/
		return false;
		/*A06_V code for P250221-05220 by shanxinkai at 20250226 end*/
	} else {
...
static int mtk_charger_plug_out(struct mtk_charger *info)
{
	...
	/*A06_V code for SR-AL7160V-01-455 by yexuedong at 20240912 start*/
	/*A14 code for R-AL6528V-01-40 by chenyulin at 20240914 start*/
	#if IS_ENABLED(CONFIG_HQ_PROJECT_O8) ||IS_ENABLED(CONFIG_HQ_PROJECT_O22)
	/*A14 code for R-AL6528V-01-40 by chenyulin at 20240914 end*/
	info->gxy_discharge_store_mode_flag = false;
	#endif
	/*A06_V code for SR-AL7160V-01-455 by yexuedong at 20240912 end*/
	/*A06_V code for SR-AL7160V-01-179 by yexuedong at 20240920 start*/
...
/*a06 code for SR-AL7160V-01-62 by wenghailong at 20240826 start*/
#if defined(CONFIG_HQ_PROJECT_O8) || defined(CONFIG_HQ_PROJECT_O22)
	usb_check_notifier_call_chain(TP_USB_PLUGOUT_STATE, NULL);
#endif//CONFIG_HQ_PROJECT_O8
/*a06 code for SR-AL7160V-01-62 by wenghailong at 20240826 end*/
	/*A06_V code for SR-AL7160V-01-161 by xiongxiaoliang at 20240919 start*/
	gxy_ttf_work_cancel();
...
/*a06 code for SR-AL7160V-01-62 by wenghailong at 20240826 start*/
#if defined(CONFIG_HQ_PROJECT_O8) || defined(CONFIG_HQ_PROJECT_O22)
	usb_check_notifier_call_chain(TP_USB_PLUGIN_STATE, NULL);
#endif//CONFIG_HQ_PROJECT_O8
/*a06 code for SR-AL7160V-01-62 by wenghailong at 20240826 end*/
	/*A06_V code for SR-AL7160V-01-161 by xiongxiaoliang at 20240919 start*/
	gxy_ttf_work_start();
...
/*A06_V code for AL7160AV-409 by shanxinkai at 20250225 start*/
#if defined(CONFIG_ODM_CUSTOM_BUILD)
					chr_err("%s, not in suspend, shutdown\n", __func__);
					kernel_power_off();
#else
					chr_err("%s, is sep! cannot shutdown\n", __func__);
#endif
/*A06_V code for AL7160AV-409 by shanxinkai at 20250225 end*/
					break;
				} else {
			...
			const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		...
		if ((val->intval > 0) && (info->enable_afc_charging))
...
static void lcm_sts_switch(bool onoff)
{
	...
	chr_err("%s: onoff[%d], lcm_sts[%d]\n", __func__, onoff, pinfo->lcm_sts);
}

static int lcm_sts_fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	int *evdata = (int *)data;
	int blank = 0;
...
static struct notifier_block lcm_sts_fb_notifier = {
	.notifier_call = lcm_sts_fb_notifier_callback,
};
#endif
/*A06_V code for SR-AL7160V-01-201 by xiongxiaoliang at 20240911 end*/
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	/*A06_V code for SR-AL7160V-01-132 by xiongxiaoliang at 20240820 start*/
	#if IS_ENABLED(CONFIG_HQ_PROJECT_O8) || IS_ENABLED(CONFIG_HQ_PROJECT_O22)
	chan_vbus = devm_iio_channel_get(&pdev->dev, "pmic_vbus");
	if (IS_ERR(chan_vbus)) {
		chr_err("charger type requests probe deferral ret:%p\n", chan_vbus);
		return -EPROBE_DEFER;
	}
	...
	if (IS_ERR(info->chan_vbus)) {
		chr_err("chan_vbus auxadc get fail, ret=%ld\n",
	...
	pinfo = info;
	info->ap_temp = 0xffff;
	info->lcm_sts = false;  //temporarily force lcd off status
	if (mtk_disp_notifier_register("Charger_lcd", &lcm_sts_fb_notifier)) {
		chr_err("mtk_disp_notifier_register failed\n");
	}
	#endif
	/*A06_V code for SR-AL7160V-01-201 by xiongxiaoliang at 20240911 end*/
	/*A06_V code for SR-AL7160V-01-179 by yexuedong at 20240920 start*/
