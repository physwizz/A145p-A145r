...
static const unsigned int __maybe_unused BOOST_VOLT_LIMIT[] = { 4850000, 5000000, 5150000,
                         5300000 };
 ...
 *********************************************************/
//gaox 
static int __upm6910_read_reg(struct upm6910x_device *upm, u8 reg, u8 *data)
{
    s32 ret;
...
static int __upm6910_write_reg(struct upm6910x_device *upm, int reg, u8 val)
{
    s32 ret;
    /* hs14 code for SR-AL6528A-01-787 by gaozhengwei at 2022/12/06 start */
    int i;
...
static int upm6910_read_byte(struct upm6910x_device *upm, u8 reg, u8 *data)
{
    int ret;
...
static int upm6910_write_byte(struct upm6910x_device *upm, u8 reg, u8 data)
{
    int ret;
...
enum upm6910x_usbsw {
 ...
 static int upm6910x_chg_set_usbsw(struct upm6910x_device *upm,
                enum upm6910x_usbsw usbsw)
{
    struct phy *phy;
...
static int upm6910x_set_vac_ovp(struct upm6910x_device *upm, int volt)
{
    ...
    /*A14 code for AL6528AV-20 by chenyulin at 20241118 start*/
    if (upm->hiz_mode_flag == true) {
        pr_err("%s now in hiz mode, return", __func__);
        return 0;
    }
    /*A14 code for AL6528AV-20 by chenyulin at 20241118 end*/
...
static int upm6910_get_input_volt_limit(struct upm6910x_device *upm, int *volt)
{
    u8 val;
    int ret;
    int vindpm;
...
int upm6910_set_input_volt_limit(struct upm6910x_device *upm, int volt)
{
    u8 val;
    ...
    return upm6910x_update_bits(upm, UPM6910_REG_06, REG06_VINDPM_MASK,
                   val << REG06_VINDPM_SHIFT);
}

int upm6910_set_chargecurrent(struct upm6910x_device *upm, int curr)
{
    u8 ichg;
    ...
    return upm6910x_update_bits(upm, UPM6910_REG_02, REG02_ICHG_MASK,
                   ichg << REG02_ICHG_SHIFT);

}

int upm6910_set_input_current_limit(struct upm6910x_device *upm, int curr)
{
    u8 val;
    ...
    return upm6910x_update_bits(upm, UPM6910_REG_00, REG00_IINLIM_MASK,
                   val << REG00_IINLIM_SHIFT);
}

static int upm6910_vbus_source_capability_detect(struct upm6910x_device *upm, int *type)
{
    int ret = 0;
    int vindpm_stat = 0;
    u8 reg_val = 0;
...
static int upm6910_dpdm_set_hidden_mode(struct upm6910x_device *upm)
{
    int ret = 0;
...
static void upm6910_bc12_recheck_dwork_handler(struct work_struct *work)
{
    ...
    int vbus = 0;
	struct delayed_work *bc12_recheck_dwork = NULL;
	struct upm6910x_device * upm = NULL;

	bc12_recheck_dwork = container_of(work, struct delayed_work, work);
	if (bc12_recheck_dwork == NULL) {
		pr_err("Cann't get hvdcp_done_work\n");
		return;
	}

	upm = container_of(bc12_recheck_dwork, struct upm6910x_device,
			bc12_recheck_dwork);
    ...
    //Charger_Detect_Init();
	//upm6910x_chg_set_usbsw(upm, USBSW_CHG);
...
/*hs14 code for AL6528ADEU-2064 by lina at 2022/11/17 start */
    ...
    if (upm->psy_usb_type != POWER_SUPPLY_USB_TYPE_DCP) {
		...
		//upm6910x_chg_set_usbsw(upm, USBSW_USB);
    } else {
        ret = upm6910_enable_hvdcp(upm, true);
        if (ret)
            pr_err("Failed to en HVDCP, ret = %d\n", ret);
...
static void upm6910x_force_detection_dwork_handler(struct work_struct *work)
{
    ...
    if (upm->state.online) {
		...
		//if (upm->psy_usb_type != POWER_SUPPLY_USB_TYPE_SDP || upm->upm_real_type == NON_STAD) {
        /*A14 code for AL6528AV-25 by chenyulin at 2024/12/6 start*/
	schedule_delayed_work(&upm->charge_detect_delayed_work, msecs_to_jiffies(20));
        /*A14 code for AL6528AV-25 by chenyulin at 2024/12/6 end*/
		//}
    } else if ((upm->state.online == false) && (upm->typec_attached == ATTACH_TYPE_TYPEC)) {
...
/*A14 code for AL6528AV-20 by chenyulin at 20241118 start*/
static int upm6910x_set_hiz_mode(struct charger_device *chg_dev, bool en)
{
    struct upm6910x_device *upm = charger_get_data(chg_dev);
    int ret = 0;

    if (!en) {
        ret += upm6910x_set_vac_ovp(upm, VAC_OVP_6500);
        ret += upm6910_set_input_volt_limit(upm, HIZ_CONTROL_VINDPM);
        upm->hiz_mode_flag = true;
    } else {
        upm->hiz_mode_flag = false;
        ret += upm6910x_set_vac_ovp(upm, VAC_OVP_10500);
        ret += upm6910_set_input_volt_limit(upm, POWER_DETECT_VINDPM);
    }
...
static int upm6910x_enable_port_charging(struct charger_device *chg_dev, bool enable)
{
    ...
    ret = upm6910x_set_hiz_mode(chg_dev, enable);
    /*A14 code for AL6528AV-20 by chenyulin at 20241118 end*/
    return ret;
}
...
static int upm6910x_hw_dev_rev_detect(struct upm6910x_device *upm)
{
    ...
    if (val == UPM6910_DEV_REV_DH) {
        gxy_bat_set_chginfo(GXY_BAT_CHG_INFO_UPM6910X);
...
static int upm6910x_parse_dt(struct upm6910x_device *upm)
{
    ...
    irq_gpio = of_get_named_gpio(upm->dev->of_node, "upm6910x,intr_gpio", 0);
...
static void upm6910x_charger_remove(struct i2c_client *client)
{
