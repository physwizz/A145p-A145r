...
#define USB_THERMAL_DEFAULT 25000      //default temp is 25°
...
static void vbus_ctrl_state(int state)
{
	...
	if(g_vbus_ctrl->state == state) {
		pr_debug("keep vbus control gpio state(%d)\n", state);
		return;
	}
...
static int update_usb_connector_state(void)
{
	...
	thermal_dev = thermal_zone_get_zone_by_name("mtktsflash");
	if (IS_ERR(thermal_dev)) {
		pr_err("%s: Unable to get thermal zone for mtktsflash\n", __func__);
		usb_temp = USB_THERMAL_DEFAULT;
	}
	ret = thermal_zone_get_temp(thermal_dev, &usb_temp);
	if (ret) {
		pr_err("%s: Failed to get thermal zone temperature", __func__);
		usb_temp = USB_THERMAL_DEFAULT;
	}
	usb_temp /= 1000;
	pr_err("[%s]: mtktsusb = %d\n", __func__, usb_temp);
