...
/************External interface**************/
struct gxy_cp_ops g_gxy_cp_ops;
EXPORT_SYMBOL(g_gxy_cp_ops);

struct gxy_cp_info g_cp_charging_lmt = {
    .cp_chg_enable = 0,
    .cp_jeita_stop_chg = 0,
...
EXPORT_SYMBOL(g_cp_charging_lmt);
/********************************************/

/****************Charge Pump API*****************/
/*A06_V code for SR-AL7160V-01-92 by xiongxiaoliang at 20240904 start*/
void usbpd_pm_set_otg_txmode(int enable)
{
    int ret = 0;
    if (g_gxy_cp_ops.set_otg_txmode) {
        ret = g_gxy_cp_ops.set_otg_txmode(&enable);
        if (ret) {
            pr_err("set otg txmode ok [%d]\n", ret);
            return;
        }
    }
    pr_err("set otg txmode fail [%d]\n", ret);
}
EXPORT_SYMBOL(usbpd_pm_set_otg_txmode);

void usbpd_pm_get_otg_txmode(int *enable)
{
    int result = 0;
    int ret = 0;
    if (g_gxy_cp_ops.get_otg_txmode) {
        ret = g_gxy_cp_ops.get_otg_txmode(&result);
        if (ret) {
            pr_err("get otg txmode ok [%d]\n", result);
            *enable = result;
            return;
        }
    }
    pr_err("get otg txmode fail [%d]\n", ret);
}
EXPORT_SYMBOL(usbpd_pm_get_otg_txmode);
...
static void usbpd_pm_update_cp_status(struct usbpd_pm *pdpm)
{
    ...
    if (g_gxy_cp_ops.get_vbus) {
        ret = g_gxy_cp_ops.get_vbus(&value);
        if (ret) {
            pdpm->cp.vbus_volt = value;
        }
    } else {
        ret = 1;
        goto err;
    ...
    if (g_gxy_cp_ops.get_ibus) {
        ret = g_gxy_cp_ops.get_ibus(&value);
        if (ret) {
            pdpm->cp.ibus_curr = value;
        }
    } else {
        ret = 1 << 1;
        goto err;
    ...
    if (g_gxy_cp_ops.get_vbat) {
        ret = g_gxy_cp_ops.get_vbat(&value);
        if (ret) {
            pdpm->cp.vbat_volt = value;
        }
    } else {
        ret = 1 << 2;
        goto err;
    ...
    if (g_gxy_cp_ops.get_ibat) {
        ret = g_gxy_cp_ops.get_ibat(&value);
        if (ret) {
            pdpm->cp.ibat_curr = value;
        }
    } else {
        ret = 1 << 3;
        goto err;
    ...
    if (g_gxy_cp_ops.get_temp) {
        ret = g_gxy_cp_ops.get_temp(&value);
        if (ret) {
            pdpm->cp.die_temp = value;
        }
    } else {
        ret = 1 << 4;
        goto err;
    ...
    if (g_gxy_cp_ops.get_charging_enabled) {
        ret = g_gxy_cp_ops.get_charging_enabled(&enable);
        if (ret) {
    ...
    pr_err("vbus,ibus,vbat,ibat,enable[%d, %d, %d, %d, %d]\n",
        pdpm->cp.vbus_volt, pdpm->cp.ibus_curr, pdpm->cp.vbat_volt, pdpm->cp.ibat_curr, pdpm->cp.charge_enabled);

    return;

err:
    pr_err("func is null [%d]\n", ret);
}
...
static int usbpd_pm_enable_cp(struct usbpd_pm *pdpm, bool enable)
{
    ...
    if (g_gxy_cp_ops.set_charging_enabled) {
        ret = g_gxy_cp_ops.set_charging_enabled(&enable);
    } else {
        pr_err("func is null [%d]\n", ret);
    }
...
static int usbpd_pm_check_cp_enabled(struct usbpd_pm *pdpm)
{
    ...
    if (g_gxy_cp_ops.get_charging_enabled) {
        ret = g_gxy_cp_ops.get_charging_enabled(&value);
        if (ret) {
            pdpm->cp.charge_enabled = !!value;
        }
    } else {
        pr_err("func is null [%d]\n", ret);
    }
...
static int usbpd_pm_set_sw_icl(struct usbpd_pm *pdpm)
{
    int ret;
...
static int usbpd_select_pdo(struct usbpd_pm *pdpm, u32 mV, u32 mA)
{
    ...
    if (pdpm->is_pps_en_unlock == false) {
        pdpm->pd_active = false;
        pr_err("pd trigger hardreset, exit cp\n");
        return -EINVAL;
    }
...
static int usbpd_pm_sm(struct usbpd_pm *pdpm)
{
    ...
    case PD_PM_STATE_FC2_ENTRY_2:
        ...
        if (pdpm->cp.vbus_err_low || (pdpm->cp.vbus_volt <
            ...
            usbpd_select_pdo(pdpm, pdpm->request_voltage, pdpm->request_current);
            pr_err("request_voltage:%d, request_current:%d, vbus_err_low:%d\n",
                    pdpm->request_voltage, pdpm->request_current, pdpm->cp.vbus_err_low);
        } else if (pdpm->cp.vbus_err_high || (pdpm->cp.vbus_volt >
                pdpm->sw.vbat_volt * BUS_VOLT_INIT_MAX)) {
            tune_vbus_retry++;
            pdpm->request_voltage -= 20;
            usbpd_select_pdo(pdpm, pdpm->request_voltage, pdpm->request_current);
            pr_err("request_voltage:%d, request_current:%d,vbus_err_high:%d\n",
                    pdpm->request_voltage, pdpm->request_current, pdpm->cp.vbus_err_high);
        } else {
