...
/*A06 code for AL7160A-33 by jiashixian at 20240401 start*/
#define UPM6720_DEFAULT_DEVICE_ID 0
static bool s_is_upm6720_probe_success = false;
/*A06 code for AL7160A-33 by jiashixian at 20240401 end*/
...
static int upm6720_set_busovp_th(struct upm6720 *upm, chg_mode_t mode, int threshold)
{
    ...
    return upm6720_update_bits(upm, UPM6720_REG_06,UPM6720_BUS_OVP_MASK, val);
}
...
static int upm6720_enable_adc(struct upm6720 *upm, bool enable)
{
    ...
    upm_err("upm6720_enable_adc enable=%d\n",enable);
...
/*A06 code for AL7160A-860 by jiashixian at 20240511 start*/
static int upm6720_enable_vout_mask(struct upm6720 *upm, bool enable)
{
    u8 val = 0;

    if (enable)
        val = UPM6720_VOUT_PRESENT_INT_NOTPRODUCE;
    else
        val = UPM6720_VOUT_PRESENT_INT_PRODUCE;

    val <<= UPM6720_VOUT_PRESENT_INT_SHIFT;

    return upm6720_update_bits(upm, UPM6720_REG_1F,
                UPM6720_VOUT_PRESENT_INT_MASK, val);
}
/*A06 code for AL7160A-860 by jiashixian at 20240512 start*/
static int upm6720_get_vbus_present_status(struct upm6720 *upm)
{
    u8 val = 0;
    int ret;
    ...
    if (!ret) {
        upm_info("STAT REG[0x15]: 0x%.2X\n", val);
        s_vbus_present = !!(val & UPM6720_VBUS_PRESENT_STAT_MASK);
    }
    upm_err("s_vbus_present = %d\n",s_vbus_present);

    return s_vbus_present;
}
/*A06 code for AL7160A-860 by jiashixian at 20240512 start*/
/*A06 code for AL7160A-860 by jiashixian at 20240511 end*/
...
static int upm6720_init_device(struct upm6720 *upm)
{
    ...
    /*A06 code for AL7160A-860 by jiashixian at 20240511 start*/
    upm6720_enable_vout_mask(upm,true);
    /*A06 code for AL7160A-860 by jiashixian at 20240511 end*/
...
static void upm6720_set_otg_txmode(struct upm6720 *upm, int enable)
{
    int ret = 0;
    u8 val1,val2,val3;
    ...
    if (enable) {
        ...
        ret |= upm6720_update_bits(upm, UPM6720_REG_0F,UPM6720_EN_OTG_MASK, val1 <<= UPM6720_EN_OTG_SHIFT);
        ret |= upm6720_update_bits(upm, UPM6720_REG_0F,UPM6720_DIS_ACDRV_BOTH_MASK, val2 <<= UPM6720_DIS_ACDRV_BOTH_SHIFT);
        ret |= upm6720_update_bits(upm, UPM6720_REG_0F,UPM6720_ACDRV1_STAT_MASK, val3 <<= UPM6720_ACDRV1_STAT_SHIFT);
    } else {
        val1 = UPM6720_EN_OTG_DISABLE;
        val2 = UPM6720_AC2_PD_ENABLE;
        val3 = UPM6720_ACDRV1_STAT_OFF;
        ret |= upm6720_update_bits(upm, UPM6720_REG_0F,UPM6720_AC2_PD_EN_MASK, val2 <<= UPM6720_AC2_PD_EN_SHIFT);
        ret |= upm6720_update_bits(upm, UPM6720_REG_0F,UPM6720_ACDRV1_STAT_MASK, val3 <<= UPM6720_ACDRV1_STAT_SHIFT);
        ret = upm6720_update_bits(upm, UPM6720_REG_0F,UPM6720_EN_OTG_MASK, val1 <<= UPM6720_EN_OTG_SHIFT);
    }

    if (ret ) {
        pr_info("upm6720_set_otg_txmode fail ! \n");
    } else {
        pr_info("upm6720_set_otg_txmode = %d\n", enable);
    }
    /*A06 code for AL7160A-607 by shixuanxuan at 20240507 start*/
    upm->otg_tx_mode = enable;
    /*A06 code for AL7160A-607 by shixuanxuan at 20240507 end*/
}
/*A06 code for SR-AL7160A-01-265 by shixuanxuan at 20240407 end*/
/******************External interface************/
int upm6720_usbpd_get_present(int *value)
{
    if (g_upm != NULL) {
        *value = g_upm->usb_present;
        return true;
    } else {
        return false;
    }
...
int upm6720_usbpd_set_present(int *value)
{
    if (g_upm != NULL) {
        upm6720_set_present(g_upm, !!(*value));
        return true;
    } else {
        return false;
    }
...
int upm6720_usbpd_get_charging_enabled(bool *value)
{
    int ret = 0;
    bool result = false;

    if (g_upm != NULL) {
        ret = upm6720_check_charge_enabled(g_upm, &result);
        if (!ret) {
            g_upm->charge_enabled = result;
        }

        *value = g_upm->charge_enabled;
        return true;
    } else {
        return false;
    }
...
int upm6720_usbpd_set_charging_enabled(bool *value)
{
    if (g_upm != NULL) {
        upm6720_enable_charge(g_upm, !!(*value));
        upm6720_check_charge_enabled(g_upm, &g_upm->charge_enabled);
        return true;
    } else {
        return false;
    }
...
int upm6720_usbpd_get_vbus(int *value)
{
    int ret = 0;
    int result = 0;

    if (g_upm != NULL) {
        ret = upm6720_get_adc_data(g_upm, UPM_ADC_VBUS, &result);
        if (!ret) {
            g_upm->vbus_volt = result;
        }

        *value = g_upm->vbus_volt;
        return true;
    } else {
        return false;
    }
...
int upm6720_usbpd_get_ibus(int *value)
{
    int ret = 0;
    int result = 0;

    if (g_upm != NULL) {
        ret = upm6720_get_adc_data(g_upm, UPM_ADC_IBUS, &result);
        if (!ret) {
            g_upm->ibus_curr = result;
        }

        *value = g_upm->ibus_curr;
        return true;
    } else {
        return false;
    }
...
int upm6720_usbpd_get_vbat(int *value)
{
    int ret = 0;
    int result = 0;

    if (g_upm != NULL) {
        ret = upm6720_get_adc_data(g_upm, UPM_ADC_VBAT, &result);
        if (!ret) {
            g_upm->vbat_volt = result;
        }

        *value = g_upm->vbat_volt;
        return true;
    } else {
        return false;
    }
...
int upm6720_usbpd_get_ibat(int *value)
{
    int ret = 0;
    int result = 0;

    if (g_upm != NULL) {
        ret = upm6720_get_adc_data(g_upm, UPM_ADC_IBAT, &result);
        if (!ret) {
            g_upm->ibat_curr = result;
        }

        *value = g_upm->ibat_curr;
        return true;
    } else {
        return false;
    }
...
int upm6720_usbpd_get_temp(int *value)
{
    int ret = 0;
    int result = 0;

    if (g_upm != NULL) {
        ret = upm6720_get_adc_data(g_upm, UPM_ADC_TDIE, &result);
        if (!ret) {
            g_upm->die_temp = result;
        }

        *value = g_upm->die_temp;
        return true;
    } else {
        return false;
    }
...
/*A06_V code for SR-AL7160V-01-92 by xiongxiaoliang at 20240904 start*/
int upm6720_usbpd_set_otg_txmode(int *value)
{
    int enable = *value;

    if (g_upm != NULL) {
        upm6720_set_otg_txmode(g_upm, enable);
        return true;
    } else {
        return false;
    }
...
int upm6720_usbpd_get_otg_txmode(int *value)
{
    if (g_upm != NULL) {
        *value = g_upm->otg_tx_mode;
        return true;
    } else {
        return false;
    }
...
static int upm6720_charger_probe(struct i2c_client *client)
{
    ...
    if (upm->part_no != UPM6720_DEFAULT_DEVICE_ID) {
        upm_info("upm6720 probe failed, Part Num:%d\n!", upm->part_no);
        goto err_1;
    }
    ...
    //schedule_delayed_work(&upm->monitor_work, msecs_to_jiffies(3*1000));
    /*A06 code for AL7160A-33 by jiashixian at 20240419 start*/
    s_is_upm6720_probe_success = true;
    /*A06 code for AL7160A-33 by jiashixian at 20240419 end*/
    g_upm = upm;
    g_gxy_cp_ops.get_present = upm6720_usbpd_get_present;
    g_gxy_cp_ops.set_present = upm6720_usbpd_set_present;
    g_gxy_cp_ops.set_charging_enabled = upm6720_usbpd_set_charging_enabled;
    g_gxy_cp_ops.get_charging_enabled = upm6720_usbpd_get_charging_enabled;
    g_gxy_cp_ops.get_vbus = upm6720_usbpd_get_vbus;
    g_gxy_cp_ops.get_ibus = upm6720_usbpd_get_ibus;
    g_gxy_cp_ops.get_vbat = upm6720_usbpd_get_vbat;
    g_gxy_cp_ops.get_ibat = upm6720_usbpd_get_ibat;
    g_gxy_cp_ops.get_temp = upm6720_usbpd_get_temp;
    /*A06_V code for SR-AL7160V-01-92 by xiongxiaoliang at 20240904 start*/
    g_gxy_cp_ops.set_otg_txmode = upm6720_usbpd_set_otg_txmode;
...
static int upm6720_suspend(struct device *dev)
{
    ...
    if (s_is_upm6720_probe_success) {
        ...
        /*A06 code for AL7160A-860 by jiashixian at 20240512 start*/
        if(!upm6720_get_vbus_present_status(upm)) {
