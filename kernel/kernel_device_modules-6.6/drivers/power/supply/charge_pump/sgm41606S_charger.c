...
struct sgm41606S_chip *g_sgm41606S = NULL;
    ...
    for (i=0;i < ARRAY_SIZE(cp_intr_flag);i++) {
        ...
        for (j=0; j <  cp_intr_flag[i].len; j++) {
            if (flag & cp_intr_flag[i].bit[j].mask) {
                dev_err(sgm->dev,"%s, trigger :%s ret[%d]\n", __func__, cp_intr_flag[i].bit[j].name, ret);
            }
        }
    }
}
...
/*A06_V code for SR-AL7160V-01-92 by xiongxiaoliang at 20240904 end*/
/******************External interface************/
int sgm41606S_usbpd_get_present(int *value)
{
    if (g_sgm41606S != NULL) {
        *value = g_sgm41606S->usb_present;
        return true;
    } else {
        return false;
    }
...
int sgm41606S_usbpd_set_present(int *value)
{
    if (g_sgm41606S != NULL) {
        sgm41606S_set_present(g_sgm41606S, !!(*value));
        return true;
    } else {
        return false;
    }
...
int sgm41606S_usbpd_get_charging_enabled(bool *value)
{
    int ret = 0;
    bool result = false;

    if (g_sgm41606S != NULL) {
        ret = sgm41606S_check_charge_enabled(g_sgm41606S, &result);
        if (!ret) {
            g_sgm41606S->charge_enabled = result;
        }

        *value = g_sgm41606S->charge_enabled;
        return true;
    } else {
        return false;
    }
...
int sgm41606S_usbpd_set_charging_enabled(bool *value)
{
    if (g_sgm41606S != NULL) {
        sgm41606S_enable_charge(g_sgm41606S, !!(*value));
        sgm41606S_check_charge_enabled(g_sgm41606S, &g_sgm41606S->charge_enabled);
        return true;
    } else {
        return false;
    }
...
int sgm41606S_usbpd_get_vbus(int *value)
{
    int ret = 0;
    int result = 0;

    if (g_sgm41606S != NULL) {
        ret = sgm41606S_get_adc_data(g_sgm41606S, ADC_VBUS, &result);
        if (!ret) {
            g_sgm41606S->vbus_volt = result;
        }

        *value = g_sgm41606S->vbus_volt;
        return true;
    } else {
        return false;
    }
...
int sgm41606S_usbpd_get_ibus(int *value)
{
    int ret = 0;
    int result = 0;

    if (g_sgm41606S != NULL) {
        ret = sgm41606S_get_adc_data(g_sgm41606S, ADC_IBUS, &result);
        if (!ret) {
            g_sgm41606S->ibus_curr = result;
        }

        *value = g_sgm41606S->ibus_curr;
        return true;
    } else {
        return false;
    }
...
int sgm41606S_usbpd_get_vbat(int *value)
{
    int ret = 0;
    int result = 0;

    if (g_sgm41606S != NULL) {
        ret = sgm41606S_get_adc_data(g_sgm41606S, ADC_VBAT, &result);
        if (!ret) {
            g_sgm41606S->vbat_volt = result;
        }

        *value = g_sgm41606S->vbat_volt;
        return true;
    } else {
        return false;
    }
...
int sgm41606S_usbpd_get_ibat(int *value)
{
    int ret = 0;
    int result = 0;

    if (g_sgm41606S != NULL) {
        ret = sgm41606S_get_adc_data(g_sgm41606S, ADC_IBAT, &result);
        if (!ret) {
            g_sgm41606S->ibat_curr = result;
        }

        *value = g_sgm41606S->ibat_curr;
        return true;
    } else {
        return false;
    }
...
int sgm41606S_usbpd_get_temp(int *value)
{
    int ret = 0;
    int result = 0;

    if (g_sgm41606S != NULL) {
        ret = sgm41606S_get_adc_data(g_sgm41606S, ADC_TDIE, &result);
        if (!ret) {
            g_sgm41606S->die_temp = result;
        }

        *value = g_sgm41606S->die_temp;
        return true;
    } else {
        return false;
    }
...
/*A06_V code for SR-AL7160V-01-92 by xiongxiaoliang at 20240904 start*/
int sgm41606S_usbpd_set_otg_txmode(int *value)
{
    int enable = *value;

    if (g_sgm41606S != NULL) {
        sgm41606S_set_otg_txmode(g_sgm41606S, enable);
        return true;
    } else {
        return false;
    }
...
int sgm41606S_usbpd_get_otg_txmode(int *value)
{
    if (g_sgm41606S != NULL) {
        *value = g_sgm41606S->otg_tx_mode;
        return true;
    } else {
        return false;
    }
...
static struct of_device_id sgm41606S_charger_match_table[] = {
    ...
    /*A06_V code for AL7160AV-21 by xiongxiaoliang at 20240918 start*/
    {},
    /*A06_V code for AL7160AV-21 by xiongxiaoliang at 20240918 end*/
};
...
static int sgm41606S_charger_probe(struct i2c_client *client)
{
    ...
    g_sgm41606S = sgm;
    g_gxy_cp_ops.get_present = sgm41606S_usbpd_get_present;
    g_gxy_cp_ops.set_present = sgm41606S_usbpd_set_present;
    g_gxy_cp_ops.set_charging_enabled = sgm41606S_usbpd_set_charging_enabled;
    g_gxy_cp_ops.get_charging_enabled = sgm41606S_usbpd_get_charging_enabled;
    g_gxy_cp_ops.get_vbus = sgm41606S_usbpd_get_vbus;
    g_gxy_cp_ops.get_ibus = sgm41606S_usbpd_get_ibus;
    g_gxy_cp_ops.get_vbat = sgm41606S_usbpd_get_vbat;
    g_gxy_cp_ops.get_ibat = sgm41606S_usbpd_get_ibat;
    g_gxy_cp_ops.get_temp = sgm41606S_usbpd_get_temp;
    /*A06_V code for SR-AL7160V-01-92 by xiongxiaoliang at 20240904 start*/
    g_gxy_cp_ops.set_otg_txmode = sgm41606S_usbpd_set_otg_txmode;
