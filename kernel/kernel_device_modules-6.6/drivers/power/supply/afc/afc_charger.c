...
extern void gxy_bat_set_afc_disable(int afc_disable);
                    ...
                    const char *buf, size_t count)
{
    ...
    if (!IS_ERR_OR_NULL(psy)) {
        ...
        /*A14 code add for P241227-01958 by chenyulin at 20250106 start*/
        gxy_bat_set_afc_disable(afc->afc_disable);
        /*A14 code add for P241227-01958 by chenyulin at 20250106 end*/
    } else {
...
static int afc_plugout_reset(struct chg_alg_device *alg)
{
    ...
    int cnt = 0;
    /*Tab A9 code for AX6739A-409 by wenyaqi at 20230530 end*/
    /*A06_V code for P250122-04729 by shanxinkai at 20250215 start*/
    int cnt_max = 5;
    /*A06_V code for P250122-04729 by shanxinkai at 20250215 end*/
    struct afc_charger *afc = NULL;
...
static int afc_set_voltage(struct chg_alg_device *alg, u32 voltage)
{
    ...
    if (ret < 0) {
        afc_err("AFC: start mping NACK, ret = %d\n", ret);
        goto out;
    }
...
static int afc_charger_probe(struct platform_device *pdev)
{
    ...
    /*A14 code add for P250219-03701 by shanxinkai at 20250225 start*/
    mutex_init(&afc->cable_out_lock);
    /*A14 code add for P250219-03701 by shanxinkai at 20250225 end*/

    afc->ta_vchr_org = SET_5V;

    afc_charger_parse_dt(afc, &pdev->dev);
    //afc->bat_psy = devm_power_supply_get_by_phandle(&pdev->dev, "gauge");
    afc->bat_psy = power_supply_get_by_name("battery");
