...
/*A06_V code for AL7160AV-497 by jiashixian at 20250325 start*/
#ifdef CONFIG_HQ_PROJECT_O8
#define LCD_ON_THERMAL_HCURRENT_UA 700000
#define LCD_ON_THERMAL_MCURRENT_UA 600000
#define LCD_ON_THERMAL_LCURRENT_UA  500000
#define LCD_OFF_THERMAL_HCURRENT_UA 2500000
#define LCD_OFF_THERMAL_MCURRENT_UA 2000000
#define LCD_OFF_THERMAL_LCURRENT_UA 1500000
#endif
/*A06 code for AL7160AV-497 by jiashixian at 20250325 end*/
...
static int gxy_battery_aging_update(struct mtk_charger *info)
{
	...
	if (s_info->gm1 != NULL) {
		cycle_count = s_info->gm1->bat_cycle;
	} else {
		chr_err("%s: s_info->gm1 NULL\n", __func__);
	}

	if (s_info->gm1 != NULL && s_info->gm1->bat_cycle_debug != 0) {
		cycle_count = s_info->gm1->bat_cycle_debug;
...
static void select_cv(struct mtk_charger *info)
{
	...
	/*A14 code for SR-AL6528V-01-26 by zhangziyi at 20240929 start*/
	#if IS_ENABLED(CONFIG_HQ_PROJECT_O8) || IS_ENABLED(CONFIG_HQ_PROJECT_O22)
	chr_err("[%s] [BATTERY_CV] gxy_batt_aging_enable:%d\n",
		__func__, info->data.gxy_batt_aging_enable);
	...
	struct chg_limit_setting *setting)
{
	...
	if (!info->lcm_sts) {
		if (pdata->thermal_charging_current_limit != -1) {
			if (pdata->thermal_charging_current_limit <=
				pdata->charging_current_limit) {
				if (pdata->thermal_charging_current_limit == LCD_ON_THERMAL_HCURRENT_UA){
					pdata->charging_current_limit = LCD_OFF_THERMAL_HCURRENT_UA;
					info->setting.charging_current_limit1 = LCD_OFF_THERMAL_HCURRENT_UA;
				} else if (pdata->thermal_charging_current_limit == LCD_ON_THERMAL_MCURRENT_UA) {
					pdata->charging_current_limit = LCD_OFF_THERMAL_MCURRENT_UA;
					info->setting.charging_current_limit1 = LCD_OFF_THERMAL_MCURRENT_UA;
				} else if (pdata->thermal_charging_current_limit == LCD_ON_THERMAL_LCURRENT_UA) {
					pdata->charging_current_limit = LCD_OFF_THERMAL_LCURRENT_UA;
					info->setting.charging_current_limit1 = LCD_OFF_THERMAL_LCURRENT_UA;
				} else {
					pdata->charging_current_limit = LCD_OFF_THERMAL_LCURRENT_UA;
					info->setting.charging_current_limit1 = LCD_OFF_THERMAL_LCURRENT_UA;
				}
	...
	#endif
	/*A06_V code for AL7160AV-497 by jiashixian at 20250325 end*/
