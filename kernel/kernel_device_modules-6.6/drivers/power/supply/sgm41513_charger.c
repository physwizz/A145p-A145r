 ...
 *
 *********************************************************/
/*A14_V code for SR-AL7160V-01-163/AL6528A-12 by shanxinkai at 20240926 start*/
#define SGM415XX_REG_NUM    (0xD)
/*A14_V code for SR-AL7160V-01-163/AL6528A-12 by shanxinkai at 20240926 end*/
...
static int sgm415xx_chg_attach_pre_process(struct sgm415xx_device *sgm, int attach)
{
    ...
    switch(attach) {
        case ATTACH_TYPE_TYPEC:
            schedule_delayed_work(&sgm->force_detect_dwork,  msecs_to_jiffies(150));
            break;
    ...
    sgm->state.chrg_type = sgm->chg_type;
...
static int sgm415xx_set_chrg_volt(struct charger_device *chg_dev, u32 uV)
{
    ...
    val1 = ((uV - SGM415XX_VREG_V_MIN_uV) % SGM415XX_VREG_V_STEP_uV) / SGM415XX_VREG_V_FT_LSB;
    if (val1 >= 1) {
        val1 = 1;
    } else {
        val1 = 0;
    }
    ret = sgm415xx_update_bits(sgm, SGM415XX_CHRG_CTRL_f, SGM415XX_VREG_V_FT_MASK,
                   val1 << SGM415XX_VREG_V_FT_SHIFT);
    if (ret < 0) {
        pr_err("[%s] write SGM41513_REG_0F fail, ret:%d\n", __func__, ret);
    }
...
static int sgm415xx_get_chrg_volt(struct charger_device *chg_dev, unsigned int *volt)
{
    ...
    ret = sgm415xx_read_reg(sgm, SGM415XX_CHRG_CTRL_f, &vreg_val2);
    if (ret) {
        return ret;
    }

    vreg_val2 = (vreg_val2 & SGM415XX_VREG_V_FT_MASK) >> SGM415XX_VREG_V_FT_SHIFT;
    if (vreg_val2 == 1) {
        *volt += SGM415XX_VREG_V_FT_LSB * 1000;
    }
    /*A06_V code for SR-AL7160V-01-163 by xiongxiaoliang at 20240912 end*/
    return 0;
}
...
static int sgm415xx_chg_set_usbsw(struct  sgm415xx_device *sgm,
                enum sgm415xx_usbsw usbsw)
{
    struct phy *phy;
    int ret, mode = (usbsw == USBSW_CHG) ? PHY_MODE_BC11_SET :
                           PHY_MODE_BC11_CLR;

    dev_err(sgm->dev, "usbsw=%d\n", usbsw);
...
static void sgm415xx_force_detection_dwork_handler(struct work_struct *work)
{
    ...
    /*A14_V code for AL6528AV-20 by chenyulin at 20241114 start*/
    static struct mtk_charger *s_info = NULL;
    struct power_supply *chg_psy = NULL;
    /*A14_V code for AL6528AV-20 by chenyulin at 20241114 end*/
    ...
    if (sgm->vbus_gd && (sgm->typec_attached == ATTACH_TYPE_TYPEC)) {
        ...
        if (s_info->hv_voltage_switch1) {
            pr_info("%s: PD enter,skip bc12\n", __func__);
            return;
        }
        /*A14 code for AL6528AV-20 by chenyulin at 20241114 end*/
        sgm415xx_chg_set_usbsw(sgm, USBSW_CHG);
        msleep(100);
        ...
        /*A14 code for AL6528AV-21 by chenyulin at 20241115 start*/
        sgm->force_dpdm_done = true;
        /*A14 code for AL6528AV-21 by chenyulin at 20241115 end*/
    } else if ((sgm->vbus_gd == false) && (sgm->typec_attached == ATTACH_TYPE_TYPEC)) {
        pr_info("[%s] a good VBUS is not attached, wait~~~\n", __func__);
        schedule_delayed_work(&sgm->force_detect_dwork,  msecs_to_jiffies(10));
    }
    ...
    switch (state->chrg_type) {
        ...
        case SGM415XX_UNKNOWN:
            ...
            if (sgm->force_detect_count < 10) {
                schedule_delayed_work(&sgm->force_detect_dwork, msecs_to_jiffies(100));
            }
            break;
        case SGM415XX_NON_STANDARD:
            pr_err("[%s] SGM415XX charger type: NON_STANDARD\n", __func__);
            sgm->chg_type = POWER_SUPPLY_TYPE_USB_DCP;
            sgm->psy_usb_type = POWER_SUPPLY_USB_TYPE_DCP;
            break;
        default:
...
static int sgm415xx_get_chrg_stat(struct sgm415xx_device *sgm)
{
    ...
    /*A14 code for AL6528AV-20 by chenyulin at 20241114 start*/
    pr_err("[%s] chrg_value=%d, chrg_stat=%d, chrg_type=%d\n", __func__,
    chrg_value, chrg_stat, sgm->state.chrg_type);
    /*A14 code for AL6528AV-20 by chenyulin at 20241114 end*/
    return chrg_value;
}
 ...
 *
 *********************************************************/
/*A14_V code for AL6528A-12 by shanxinkai at 20240926 start*/
static int sgm415xx_dump_register(struct charger_device *chg_dev)
{
    ...
    ret = sgm415xx_read_reg(sgm, SGM415XX_CHRG_CTRL_f, &val);
    if (ret != 0) {
        pr_info("[%s] i2c transfor error\n", __func__);
        return 1;
    }
...
/*A14_V code for AL6528A-12 by shanxinkai at 20240926 end*/
...
static void sgm415xx_charger_detect_work_func(struct work_struct *work)
{
    ...
    if (sgm->state.chrg_type != SGM415XX_USB_DCP) {
...
static irqreturn_t sgm415xx_irq_handler_thread(int irq, void *private)
{
    ...
    /*A14 code for AL6528AV-20 by chenyulin at 20241114 start*/
    pr_err("[%s] vbus_gd_pre=%d, vbus_gd=%d, power_good_pre=%d, power_good=%d, chg_type=%d\n", __func__,
        vbus_gd_pre, sgm->vbus_gd, power_good_pre, sgm->power_good, sgm->chg_type);
    /*A14 code for AL6528AV-20 by chenyulin at 20241114 end*/
    ...
    if (!vbus_gd_pre && sgm->vbus_gd) {
        ...
        /*A06_V code for SR-AL7160V-01-138 by xiongxiaoliang at 20240910 start*/
        sgm->force_recheck = false;
        /*A06_V code for SR-AL7160V-01-138 by xiongxiaoliang at 20240910 end*/
        /*A14 code for AL6528AV-21 by chenyulin at 20241115 start*/
        sgm->force_dpdm_done = false;
        /*A14 code for AL6528AV-21 by chenyulin at 20241115 end*/
        return IRQ_HANDLED;
    }

    /* avoid PD power_off_charging reboot, and AC exit HIZ need redo HVDCP */
    /*A14 code for AL6528AV-20|AL6528AV-21 by chenyulin at 20241115 start*/
    if ((sgm->typec_attached == ATTACH_TYPE_TYPEC) &&
        (sgm->force_dpdm_done == true) && (sgm->power_good)) {
        sgm->force_dpdm_done = false;
    /*A14 code for AL6528AV-20|AL6528AV-21 by chenyulin at 20241115 end*/
        pr_info("[%s] trigger BC12 irq\n", __func__);
        schedule_delayed_work(&sgm->charge_detect_delayed_work, msecs_to_jiffies(10));
    }

    /*A06_V code for SR-AL7160V-01-138 by xiongxiaoliang at 20240910 start*/
    if (sgm->force_recheck == true) {
        pr_info("[%s] force recheck irq\n", __func__);
        sgm->force_recheck = false;
        schedule_delayed_work(&sgm->charge_detect_delayed_work, msecs_to_jiffies(10));
    }
    ...
    /*A14 code for AL6528AV-20 by chenyulin at 20241114 start*/
    if ((sgm->typec_attached == ATTACH_TYPE_TYPEC) &&
        (sgm->state.chrg_type == SGM415XX_NON_STANDARD) &&
        (sgm->force_detect_count < 2)) {
    /*A14 code for AL6528AV-20 by chenyulin at 20241114 end*/
        pr_err("[%s] NON_STANDARD, recheck\n", __func__);
        sgm->force_recheck = true;
        schedule_delayed_work(&sgm->force_detect_dwork, msecs_to_jiffies(100));
    }
    /*A06_V code for SR-AL7160V-01-138 by xiongxiaoliang at 20240910 end*/
    /*A14 code for AL6528AV-20 by chenyulin at 20241114 start*/
...
static int sgm415xx_hw_init(struct sgm415xx_device *sgm)
{
    ...
    ret = sgm415xx_set_recharge_volt(sgm, 100);//100~200mv
    if (ret)
        goto err_out;
...
static int sgm415xx_is_enabled_vbus(struct regulator_dev *rdev)
{
    ...
    sgm415xx_read_reg(sgm, SGM415XX_CHRG_CTRL_1, &temp);
...
static int sgm415xx_driver_probe(struct i2c_client *client)
{
    ...
    gxy_bat_set_chginfo(GXY_BAT_CHG_INFO_SGM41513DYTQF24G);
    ...
    INIT_DELAYED_WORK(&sgm->charge_detect_delayed_work, sgm415xx_charger_detect_work_func);
