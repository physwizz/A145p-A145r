...
static int gxt_ttf_tcp_notifier_call(struct notifier_block *nb,
                    unsigned long event, void *data)
{
    struct gxy_battery_ttf *battery = container_of(nb, struct gxy_battery_ttf, tcp_nb);
    struct tcp_notify *noti = data;

    GXY_PSY_ERR("event:%ld, connected:%d\n", event, noti->pd_state.connected);

    switch (event) {
        case TCP_NOTIFY_PD_STATE:
            battery->pd_type = noti->pd_state.connected;
            /*A06_V code for SR-AL7160V-01-152 by xiongxiaoliang at 20240924 start*/
            if (battery->chg_info != NULL) {
                battery->chg_info->chg_pd_type = battery->pd_type;
            } else {
                GXY_PSY_ERR("chg_info is NULL\n");
            }
...
static void gxy_ttf_register_tcpc(struct gxy_battery_ttf *battery)
{
    int ret = 0;

    if (!battery->tcpc) {
        battery->tcpc = tcpc_dev_get_by_name("type_c_port0");
        if (!battery->tcpc) {
            GXY_PSY_ERR("get tcpc dev fail\n");
            return;
        }
        battery->tcp_nb.notifier_call = gxt_ttf_tcp_notifier_call;
        ret = register_tcp_dev_notifier(battery->tcpc, &battery->tcp_nb,
                        TCP_NOTIFY_TYPE_USB);
        if (ret < 0) {
            GXY_PSY_ERR("register tcpc notifier fail\n");
        }
    }
}
...
static int gxy_calc_ttf_to_full_capacity(struct gxy_battery_ttf *battery, unsigned int ttf_curr)
{
    ...
    int charge_current = ttf_curr;
    int design_cap = battery->ttf_d->ttf_capacity;
    /*A14V code for AL6528AV-142 by chenyulin at 20250113 start*/
    ...
    /*A14V code for AL6528AV-142 by chenyulin at 20250113 end*/
    if (!cv_data || (ttf_curr <= 0)) {
        GXY_PSY_ERR("%s: no cv_data or val: %d\n", __func__, ttf_curr);
        return -1;
    }
...
static void gxy_bat_calc_time_to_full(struct gxy_battery_ttf *battery)
{
    ...
    if (delayed_work_pending(&battery->ttf_d->timetofull_work)) {
        ...
        GXY_PSY_ERR("%s: usb_vol_now[%d], batt_curr_now[%d], usb_type[%d pd:%d], max_charge_power[%d]\n", __func__,
            battery->usb_vol_now, battery->batt_curr_now, battery->usb_type, battery->pd_type, battery->max_charge_power);

        // STEP1: USB TYPE judge
        if ((info->enable_hv_charging == true) && (info->afc_result == 1 ||
            battery->pd_type == PD_CONNECT_PE_READY_SNK ||
            battery->pd_type == PD_CONNECT_PE_READY_SNK_PD30 ||
            battery->pd_type == PD_CONNECT_PE_READY_SNK_APDO)) { /* high voltage charging */
                charge = battery->ttf_d->ttf_hv_charge_current;
                if (battery->pd_type == PD_CONNECT_PE_READY_SNK_APDO) {
                    charge = battery->ttf_d->ttf_pd25_charge_current;
                }
        } else if (battery->usb_type == POWER_SUPPLY_USB_TYPE_DCP){ /* other AC charging */
            if ((battery->usb_vol_now >= USB_HV_VLOTAGE_MIN) && (info->enable_hv_charging == true)) {
                charge = battery->ttf_d->ttf_hv_charge_current;
            } else {
        ...
        if (battery->saved_h_min_time != (h_time * 60 + min_time)) {
            ...
            if ((battery->capacity >= 98) && (battery->capacity < 100) && charge >= 307) {
                ...
                GXY_PSY_ERR("%s: ts.tv_sec:%lld, charging_start_time=%lld, charging_passed_time:%lld, saved_charging_passed_time:%d\n", __func__,
        ...
        //force display 1 min when UISOC < 100%.
        if (battery->ttf_d->timetofull <= 60) {
            battery->ttf_d->timetofull = 60;
        }
...
static void gxy_bat_time_to_full_work(struct work_struct *work)
{
    ...
    if (gxy_ttf_init_psy(gxy_battery) == false) {
        ...
        if (check_ttf_state(gxy_battery->capacity, gxy_battery->bat_sts)) {
            ...
            if ((gxy_battery->capacity >= 80) && (gxy_battery->chg_info != NULL) &&
                (gxy_battery->chg_info->gxy_discharge_batt_full_capacity > 0 && gxy_battery->chg_info->gxy_discharge_batt_full_capacity != 100)) {
                queue_delayed_work(gxy_battery->monitor_wqueue,
                    &gxy_battery->ttf_d->timetofull_work, msecs_to_jiffies(2000));
                GXY_PSY_ERR("%s: switch %d protect, Continue working\n", __func__, gxy_battery->chg_info->gxy_discharge_batt_full_capacity);
            } else {
                GXY_PSY_ERR("%s: cancel work\n", __func__);
                cancel_delayed_work(&gxy_battery->ttf_d->timetofull_work);
            }
        }
    }
...
int gxy_ttf_init(void)
{
    ...
    gxy_ttf_register_tcpc(gxy_battery);
    return 0;
