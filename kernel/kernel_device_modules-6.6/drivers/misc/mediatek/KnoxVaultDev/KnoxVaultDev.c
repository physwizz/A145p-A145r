...
#endif
    if (!dev || !attr || !buf || !len) {
        pr_err("[%s]l=%d: dev, attr, buf is NULL, len=%lu\n",
		      ...
		      char *devname, char *classname)
{
    ...
    k250a_dev->k250a_class = class_create(classname);
    ...
    k250a_dev->k250a_device = device_create(k250a_dev->k250a_class, NULL,
        k250a_dev->devno, k250a_dev, K250A_CHAR_DEV_NAME);
			 ...
			 size_t count, loff_t *offset)
{
    ...
    pr_info("[%s]l=%d: NONBLOCK read count=%lu, ret=%d\n", __FUNCTION__, __LINE__, count, ret);
    // } else {
#if defined(READ_i2c_SLAVE_BY_INTERRUPT)
...
static int k250a_i2c_dev_probe(struct i2c_client *client)
{
    int ret = 0;
...
static void k250a_i2c_dev_remove(struct i2c_client *client)
{
    struct k250a_dev *k250a_dev = NULL;
