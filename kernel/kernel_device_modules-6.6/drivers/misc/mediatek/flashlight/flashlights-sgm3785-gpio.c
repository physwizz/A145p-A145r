...
static DEFINE_MUTEX(sgm3785_pinctrl_mutex);
...
static int sgm3785_gpio_pinctrl_set(int pinType, int state)
{
    ...
    case SGM3785_ENM_GPIO_MODE:
        if (state == SGM3785_GPIO_PINCTRL_PINSTATE_LOW &&
            ...
            mutex_lock(&sgm3785_pinctrl_mutex);
            pinctrl_select_state(sgm_gpio_pinctrl, sgm3785_enm_gpio_l);
            mutex_unlock(&sgm3785_pinctrl_mutex);
        }
...
/* A06 code for SR-AL7160A-01-501 by zhongbin at 20240508 start */
/******************************************************************************
 * sgm3785_set_torch_mode
 *****************************************************************************/
/*hs14 code for AL6528ADEU-2059 by pengxutao at 2022-11-11 start*/
/* A06 code for AL7160A-1677  by dingling at 2024/06/14 start */
static int Count =0;
static int sgm3785_set_torch_mode(int level)
{
    ...
    pr_info("o8 torch_mode enter level=%d\n",level);
    ...
    if (level == HQ_TORCH_LEVEL_THREE){
        ...
        flashlight_config.PWM_MODE_OLD_REGS.THRESH = level * HQ_TORCH_LEVEL_THREE + HQ_TORCH_LEVEL_ONE;
    }
...
static int sgm3785_set_preflash_mode(int level)
{
    ...
    int ret;
    /* TODO: wrap enable function */
    pr_info("preflash_mode enter level=%d\n",level);
    ...
    state = SGM3785_GPIO_PINCTRL_PINSTATE_HIGH;
    ret = sgm3785_gpio_pinctrl_set(pinType, state);
// A06 code for SR-AL7160A-01-502 by xuyunhui at 20250205 start
...
static int sgm3785_set_flash_mode(int level)
{
    int pinType = 0, state = 0;
        int ret;
    /* TODO: wrap enable function */
    pr_info("sgm3785 flash_mode enter set level:%d\n",level);
...
/* A06 code for AL7160A-1677  by dingling at 2024/06/14 end */
/* flashlight init */
...
/* A06 code for SR-AL7160A-3721  by zhongquan at 20240702 start */
/******************************************************************************
...
static int sgm3785_gpio_ioctl(unsigned int cmd, unsigned long arg)
{
    ...
    case FLASH_IOC_IS_CHARGER_READY:  //210 preflash
        ...
        fl_arg->arg = 1;
        #endif
// A06 code for SR-AL7160A-01-502 by xuyunhui at 20250205 end
        flashlight_mode = MODE_PREFLASH;
        break;
    case FLASH_IOC_SET_ONOFF:              //115
        pr_info("timeout_ms = %d, current_level = %d, flashlight_mode= %d\n",
                sgm3785_gpio_timeout_ms,current_level,flashlight_mode);
        if (fl_arg->arg == 1) {
            switch(flashlight_mode) {
                case MODE_FLASH: {
                    if (sgm3785_gpio_timeout_ms == 0) {
                        sgm3785_gpio_timeout_ms = 200;
                    }
...
/* A06 code for SR-AL7160A-3721  by zhongquan at 20240702 end */
/* A06 code for SR-AL7160A-01-501 by zhongbin at 20240508 end */
