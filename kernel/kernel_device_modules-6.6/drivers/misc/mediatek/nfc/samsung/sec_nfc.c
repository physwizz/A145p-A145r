...
bool nfc_get_mode(void) {
	...
	} else if (strnstr(nfcload, "nfcinfo=NFC_SMD", strlen(nfcload))) {
		pr_info("%s: this device support NFC!", __func__);
		return true;
	}
...
static int __sec_nfc_probe(struct device *dev)
{
	...
	if (!nfc_get_mode()) {
		pr_info("This device do not support NFC_ESE, we ignore it!\n");
		return -ENODEV;
	}
...
#else /* Not defined : CONFIG_SEC_NFC_IF_I2C */
...
static void sec_nfc_remove(struct platform_device *pdev)
{
