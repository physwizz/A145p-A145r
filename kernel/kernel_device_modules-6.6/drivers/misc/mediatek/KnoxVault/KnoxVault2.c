					...
					const char *buf, size_t len)
{
    ...
    if (len <= 0) {
        pr_err("[%s]l=%d: failed to en ldo_vio18, len=%lu\n",
    ...
    if (buf[0] == '1' && k250a_data.k250a_is_open == K250A_CLOSE) {
        ...
        pr_err("[%s]l=%d: buf is %s, len is %lu, k250a_is_open is %d\n",
...
static int k250a_probe(struct platform_device *pdev)
{
    ...
    k250a_node_class = class_create("k250a_node_class");
    if (IS_ERR(k250a_node_class)) {
        pr_err("fail to create k250a class,ret is %ld\n", PTR_ERR(k250a_node_class));
    ...
    if (IS_ERR(k250a_node_device)) {
        pr_err("fail to create k250a device,ret is %ld\n", PTR_ERR(k250a_node_device));
