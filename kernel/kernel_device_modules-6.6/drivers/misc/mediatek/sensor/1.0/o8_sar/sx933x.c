...
static int smtc_i2c_write(Self self, u16 reg_addr, u32 reg_val)
{
    ...
    while(true)
    {
        ...
        {
            SMTC_LOG_ERR("i2c_addr=0x%X i2c write reg 0x%x error %d. Goint to retry", i2c->addr, reg_addr, ret);
        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (ret < 0) {
        SMTC_LOG_INF("Invalid value of input, input=%d", ret);
        return -EINVAL;
    }
...
static int smtc_parse_dts(Self self)
{
    ...
    self->gpio = of_get_named_gpio(of_node, "Semtech,nirq-gpio", 0);
...
static int smtc_probe(struct i2c_client *client)
{
    ...
    if (!self)
    {
        SMTC_LOG_ERR("Failed to create self. memory size=%ld bytes.", sizeof(self_t));
        return -ENOMEM;
    }
