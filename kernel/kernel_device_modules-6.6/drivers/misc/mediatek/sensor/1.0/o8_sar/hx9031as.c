...
//ANFR
#ifdef HX9031AS_USER_NODE
static void tyhx_anfr_irq(void)
{
    int ii = 0;
...
static void tyhx_anfr_forcereport(void)
{
    int ii = 0;
...
/*A06 code for SR-AL7160A-01-189 by jiashixian at 20240430 end*/
/*A06 code for SR-AL7160A-01-188 by jiashixian at 20240420 start*/
static void hx_tyhx_enable_report(void)
{
    int ii = 0;
    uint8_t touch_state = 0;
...
static void hx_tyhx_disable_report(void)
{
    int ii = 0;
    if (g_self != NULL) {
        for (ii = 0; ii < HX9031AS_CH_NUM; ii++) {
        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (ret < 0) {
        PRINT_INF("Invalid value of input, input=%d", ret);
        return -EINVAL;
    }
...
static void hx9031_onoff_enablereport(void)
{
    int ii = 0;
    uint8_t touch_state = 0;
...
static void hx9031_onoff_disablereport(void)
{
    int ii = 0;
...
#else
    hx9031as_pdata.irq_gpio = of_get_named_gpio(dt_node, "tyhx,irq-gpio", 0);
