...
/*a06 code for SR-AL7160V-01-44 by liuyiying at 20240830 start*/
#if IS_ENABLED(CONFIG_MTK_SMARTALERT_HUB)
struct alert_event_t {
	int32_t state;		/* 0,1 */
};
#endif
/*a06 code for SR-AL7160V-01-44 by liuyiying at 20240830 end*/
/*a06 code for SR-AL7160V-01-43 by xiayujie at 20240906 start*/
#if IS_ENABLED(CONFIG_MTK_WAKE_UP_MOTION)
struct wakeup_event_t {
	int32_t state;		/* 0,1 */
};
#endif
/*a06 code for SR-AL7160V-01-43 by xiayujie at 20240906 end*/
...
extern int scp_sensorHub_data_registration(uint8_t sensor,
	SCP_sensorHub_handler handler);
extern int sensor_enable_to_hub(uint8_t sensorType, int enabledisable);
int sensor_set_delay_to_hub(uint8_t sensorType, unsigned int delayms);
extern int sensor_get_data_from_hub(uint8_t sensorType,
	struct data_unit_t *data);
int sensor_set_cmd_to_hub(uint8_t sensorType,
	enum CUST_ACTION action, void *data);
extern int sensor_batch_to_hub(uint8_t sensorType,
	int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs);
extern int sensor_flush_to_hub(uint8_t sensorType);
