...
static int initialize(psx93XX_t this)
{
    if (this)
    {
        ...
        msleep(100);
        sx933x_global_variable_init(this);
...
static ssize_t smtc_set_dumpinfo(const char *buf, size_t count)
{
    ...
    if (sscanf(buf, "%d", &value) != 1) {
        pr_info("Invalid command format. Usage: ehco 1 > dumpinfo");
        return -EINVAL;
    }
...
static ssize_t smtc_get_dumpinfo(char *buf)
{
    ...
    if (g_sar_handle != NULL) {
        phase = &smtc_phase_table[g_channel];
        sx933x_i2c_read_16bit(g_sar_handle, SX933X_STAT0_REG, &reg_val);
        ...
        sx933x_i2c_read_16bit(g_sar_handle, SX933X_USEPH0_REG + shift, &reg_val);
        g_dumpinfo.raw_data = (s32)reg_val >> g_bitshift;
        sx933x_i2c_read_16bit(g_sar_handle, SX933X_AVGPH0_REG + shift, &reg_val);
        g_dumpinfo.baseline_data = (s32)reg_val >> g_bitshift;
        sx933x_i2c_read_16bit(g_sar_handle, SX933X_DIFFPH0_REG + shift, &reg_val);
        g_dumpinfo.diff_data = (s32)reg_val >> g_bitshift;
        sx933x_i2c_read_16bit(g_sar_handle, SX933X_OFFSETPH0_REG + shift * OFFSET_PH_REG_SHIFT, &reg_val);
        ...
        return snprintf(buf, PAGE_SIZE, "channel:%d,state:%d,total_cap:%d,ref_cap:%d,raw_data:%d,baseline_data:%d,diff:%d\n",
            g_channel, g_dumpinfo.ph_state, g_dumpinfo.total_cap, g_dumpinfo.ref_cap,
            g_dumpinfo.raw_data, g_dumpinfo.baseline_data, g_dumpinfo.diff_data);

        pr_info("ph_state=%d", g_dumpinfo.ph_state);
    } else {
        pr_info("psx93XX_t is NULL!");
    }
...
/*A14_V code for AL6528AV-10 by hefan at 2023/09/11 start*/
int machine_detcet(void)
{
    ...
    if (NULL != strstr(bootparams, "pcbainfo=EU")) {
...
/*A14_V code for AL6528AV-10 by hefan at 2023/09/11 end*/
 ...
 */
static int sx933x_probe(struct i2c_client *client)
{
    int i = 0;
    int err = 0;
    /* hs14 code for AL6528A-104 by duxinqi at 2022/09/23 start */
    ...
    if (this)
    {
        ...
        * (1->NIRQ=0, 0->NIRQ=1) */
        ...
        /*A14_V code for SR-AL6528V-01-196 by hefan at 2023/09/23 start*/
        sar_func.set_dumpinfo = smtc_set_dumpinfo;
        sar_func.get_dumpinfo = smtc_get_dumpinfo;
        /*A14_V code for SR-AL6528V-01-196 by hefan at 2023/09/23 end*/
        /*HS14_U code for SR-AL6528E-01-37 by xiayujie at 2023/07/28 end*/
        ...
        if (pDevice)
        {
            ...
            dev_err(&client->dev, "[SX933x]:pDevice is null\n");
            return -1;
        }
        ...
        dev_err(&client->dev, "[SX933x]:this is null\n");
        return -1;
    }
#if defined(SX933X_USB_CALI) || defined(SX933X_FACTORY_NODE)
    sar_this = this;
    ...
    if (err > 0) {
        ...
        if (err) {
            dev_err(&client->dev, "cound not create input sub group\n");
            return -ENOMEM;
        }
        ...
        if (err) {
            dev_err(&client->dev, "%s - cound not register sub sensor(%d).\n", __func__, err);
            return -ENOMEM;
        }
#endif
    }
...
static void sx933x_remove(struct i2c_client *client)
{
    psx933x_platform_data_t pplatData =0;
    psx933x_t pDevice = 0;
    psx93XX_t this = i2c_get_clientdata(client);
