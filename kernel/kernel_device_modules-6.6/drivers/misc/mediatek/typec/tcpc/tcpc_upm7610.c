...
#define UPM7610_DRV_VERSION	"2.0.8_MTK"
...
RT_REG_DECL(UPM7610_REG_VCONN_CTRL, 1, RT_VOLATILE, {});
...
RT_REG_DECL(UPM7610_REG_TRIM_C4, 1, RT_VOLATILE, {});
RT_REG_DECL(UPM7610_REG_TRIM_C5, 1, RT_VOLATILE, {});
...
static const rt_register_map_t upm7610_chip_regmap[] = {
	...
	RT_REG(UPM7610_REG_TRIM_C4),
	RT_REG(UPM7610_REG_TRIM_C5),
					...
					uint8_t reg_addr, uint16_t data)
{
	...
	return upm7610_block_write(client, reg_addr, 2, &data);
}

static int32_t upm7610_read_word(struct i2c_client *client,
					uint8_t reg_addr, uint16_t *data)
{
	int ret = upm7610_block_read(client, reg_addr, 2, data);
...
static int upm7610_i2c_update_bits(struct tcpc_device *tcpc, u8 reg,
				  u8 val, u8 mask)
{
	u8 data = 0;
	int ret = 0;

	ret = upm7610_i2c_read8(tcpc, reg);
	if (ret < 0)
		return ret;
	data = ret;
	...
	return upm7610_i2c_write8(tcpc, reg, data);
}

static inline int upm7610_i2c_set_bits(struct tcpc_device *tcpc, u8 reg, u8 mask)
{
	return upm7610_i2c_update_bits(tcpc, reg, mask, mask);
}

static inline int upm7610_i2c_clr_bits(struct tcpc_device *tcpc, u8 reg, u8 mask)
{
	return upm7610_i2c_update_bits(tcpc, reg, 0x00, mask);
}
...
static inline int upm7610_software_reset(struct tcpc_device *tcpc)
{
	...
	ret = upm7610_i2c_write8(tcpc, UPM7610_REG_RESET_CTRL, UPM7610_REG_SOFT_RESET);
	...
	/*A06_V code for SR-AL7160V-01-135 by yexuedong at 20240828 start*/
	return upm7610_i2c_write8(tcpc, UPM7610_REG_RESET_CTRL,
            UPM7610_REG_EXT_STATUS_MASK_CTRL |
            UPM7610_REG_CABLE_RESET_CTRL);
	/*A06_V code for SR-AL7160V-01-135 by yexuedong at 20240828 end*/
}
...
static inline int upm7610_init_alert_mask(struct tcpc_device *tcpc)
{
	struct upm7610_chip *chip = tcpc_get_dev_data(tcpc);
	...
	*(uint16_t *)masks = cpu_to_le16(mask);
	return upm7610_block_write(chip->client, TCPC_V10_REG_ALERT_MASK,
				  sizeof(masks), masks);
}
...
static int upm7610_rx_alert_mask(struct tcpc_device *tcpc)
{
	...
	UPM7610_INFO("rx_alert_mask:0x%x\n", ret);
	mask = (uint16_t) ret;
...
static inline int upm7610_init_up_mask(struct tcpc_device *tcpc)
{
	uint8_t rt_mask =   UPM7610_REG_VSAFE0V_STATUS_MASK |
                        //UPM7610_REG_RA_DETACH_MASK |
                        UPM7610_REG_REF_DISCNT_MASK;

	return upm7610_i2c_write8(tcpc, UPM7610_REG_VDR_DEF_ALERT_MASK, rt_mask);
}
...
static int upm7610_alert_status_clear(struct tcpc_device *tcpc, uint32_t mask)
{
	int ret;
	uint16_t mask_t1;
	uint8_t mask_t2;
...
static int upm7610_idle_ctrl(struct tcpc_device *tcpc)
{
    //struct upm7610_chip *chip = tcpc_get_dev_data(tcpc);
...
static int upm7610_tcpc_init(struct tcpc_device *tcpc, bool sw_reset)
{
	...
	upm7610_alert_status_clear(tcpc, 0xffffffff);

	upm7610_init_up_mask(tcpc);
	upm7610_init_alert_mask(tcpc);

	upm7610_idle_ctrl(tcpc);
    ...
    data = upm7610_i2c_read8(tcpc, UPM7610_REG_TRIM_C5);
    if (data < 0) {
    ...
    upm7610_i2c_write8(tcpc, UPM7610_REG_TRIM_C5, data);
...
static inline int upm7610_fault_status_vconn_ov(struct tcpc_device *tcpc)
{
	return upm7610_i2c_clr_bits(tcpc, UPM7610_REG_VCONN_CTRL,
				   UPM7610_REG_VCONN_DISCHG_EN);
}

static int upm7610_fault_status_clear(struct tcpc_device *tcpc, uint8_t status)
{
	if (status & TCPC_V10_REG_FAULT_STATUS_VCONN_OV)
		upm7610_fault_status_vconn_ov(tcpc);

	return upm7610_i2c_write8(tcpc, TCPC_V10_REG_FAULT_STATUS, status);
}

static int upm7610_set_alert_mask(struct tcpc_device *tcpc, uint32_t mask)
{
	int ret = 0;

	ret = upm7610_i2c_write16(tcpc, TCPC_V10_REG_ALERT_MASK, mask);
	if (ret < 0)
		return ret;

	return upm7610_i2c_write8(tcpc, UPM7610_REG_VDR_DEF_ALERT_MASK, mask >> 16);
}

static int upm7610_get_alert_mask(struct tcpc_device *tcpc, uint32_t *mask)
{
	int ret;
	uint8_t v2;
...
static int upm7610_get_alert_status_and_mask(struct tcpc_device *tcpc,
					    uint32_t *alert, uint32_t *mask)
{
	...
	ret = upm7610_block_read(chip->client, TCPC_V10_REG_ALERT, 4, buf);
	if (ret < 0)
		return ret;
	*alert = le16_to_cpu(*(uint16_t *)&buf[0]);
	*mask = le16_to_cpu(*(uint16_t *)&buf[2]);

	ret = upm7610_block_read(chip->client, UPM7610_REG_VDR_DEF_ALERT, 2, buf);
	if (ret < 0)
		return ret;
	*alert |= buf[0] << 16;
	*mask |= buf[1] << 16;
...
static int upm7610_get_power_status(struct tcpc_device *tcpc)
{
	int ret;
	...
	if (ret < 0)
		return ret;
	tcpc->vbus_safe0v = !!(ret & UPM7610_REG_VSAFE0V_STATUS);
	return 0;
}

static int upm7610_get_fault_status(struct tcpc_device *tcpc, uint8_t *status)
{
	int ret;
...
{
	return (enable ? upm7610_i2c_set_bits : upm7610_i2c_clr_bits)
		(tcpc, UPM7610_REG_VDR_DEF_ALERT_MASK, UPM7610_REG_VSAFE0V_STATUS_MASK);
}
...
static int upm7610_set_polarity(struct tcpc_device *tcpc, int polarity)
{
	...
	return (polarity ? upm7610_i2c_set_bits : upm7610_i2c_clr_bits)
		...
		struct tcpc_device *tcpc, bool en, int pull)
{
	...
	chip->lpm_en = !!en;
	ret = upm7610_idle_ctrl(tcpc);
	if (ret < 0)
		return ret;
	return upm7610_enable_vsafe0v_detect(tcpc, !en);
}
			...
			uint16_t *msg_head, enum tcpm_transmit_type *frame_type)
{
    ...
    /*A06_V code for AL7160AV-495 by jiashixian at 20250325 end*/
	rv = upm7610_block_read(chip->client, TCPC_V10_REG_RX_BYTE_CNT,
			       sizeof(buf), buf);
	if (rv < 0)
		return rv;
	...
	*msg_head = le16_to_cpu(*(uint16_t *)&buf[2]);
    ...
    /*A06_V code for AL7160AV-495 by jiashixian at 20250325 start*/
    //	if (*msg_head != 0x77a3)
    //		upm7610_alert_status_clear(tcpc, alert_rx);
   /*A06_V code for AL7160AV-495 by jiashixian at 20250325 end*/
	return rv;
}
...
static int upm7610_set_bist_test_mode(struct tcpc_device *tcpc, bool en)
{
	...
	return (en ? upm7610_i2c_set_bits : upm7610_i2c_clr_bits)
		(tcpc, TCPC_V10_REG_TCPC_CTRL,
...
static struct tcpc_ops upm7610_tcpc_ops = {
	...
	.set_alert_mask = upm7610_set_alert_mask,
	.get_alert_mask = upm7610_get_alert_mask,
	.get_alert_status_and_mask = upm7610_get_alert_status_and_mask,
...
static int up_parse_dt(struct upm7610_chip *chip, struct device *dev)
{
	...
	ret = of_get_named_gpio(np, "upm7610pd,intr-gpio", 0);
	if (ret < 0)
	...
	if (ret < 0)
		...
		chip->irq_gpio = ret;
#else
	ret = of_property_read_u32(np, "upm7610pd,intr-gpio-num", &chip->irq_gpio) ?
	      of_property_read_u32(np, "upm7610pd,intr_gpio_num", &chip->irq_gpio) : 0;
	if (ret < 0)
		pr_err("%s no intr_gpio info\n", __func__);
...
static inline int upm7610_check_revision(struct i2c_client *client)
{
	...
	data = UPM7610_REG_EXT_STATUS_MASK_CTRL | UPM7610_REG_CABLE_RESET_CTRL;
	ret = upm7610_write_device(client, UPM7610_REG_RESET_CTRL, 1, &data);
	if (ret < 0)
		return ret;
	/*A06_V code for SR-AL7160V-01-135 by yexuedong at 20240828 end*/
...
static int upm7610_i2c_probe(struct i2c_client *client)
{
	...
	/* set for tcpc_info */
	#if IS_ENABLED(CONFIG_HQ_PROJECT_O8)
	gxy_bat_set_tcpcinfo(GXY_BAT_TCPC_INFO_UPM7610);
	#endif
	/*A06_V code for SR-AL7160V-01-97 by yexuedong at 20240902 end*/
...
static int __init upm7610_init(void)
{
	...
	pr_info("%s (%s)\n", __func__, UPM7610_DRV_VERSION);
	np = of_find_node_by_name(NULL, "upm7610");
	pr_info("%s upm7610 node %s\n", __func__,
