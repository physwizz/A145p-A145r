...
#define WUSB3812C_DRV_VERSION	"1.9.04a_MTK"
...
static void wusb3812_irq_work_handler(struct kthread_work *work)
{
	...
	do {
		regval = tcpci_alert(chip->tcpc,true);
		if (regval)
			break;
		gpio_val = gpio_get_value(chip->irq_gpio);
		WS_DBG("gpio[%d]_val = %d\n", chip->irq_gpio, gpio_val);
		WS_DBG("R10==%#x,R11=%#x,R1A==%#x,R1D==%#x,R1E=%#x,",
			wusb3812_i2c_read8(chip->tcpc, 0X10),wusb3812_i2c_read8(chip->tcpc, 0X11),
			wusb3812_i2c_read8(chip->tcpc, 0X1A),wusb3812_i2c_read8(chip->tcpc, 0X1D),
			wusb3812_i2c_read8(chip->tcpc, 0X1E));
	} while (gpio_val == 0);
...
int wusb3812_alert_status_clear(struct tcpc_device *tcpc, uint32_t mask)
{
	int ret;
	uint16_t mask_t1;
//#if IS_ENABLED(CONFIG_TCPC_VSAFE0V_DETECT_IC)
	uint8_t mask_t2;
...
int wusb3812_get_alert_status(struct tcpc_device *tcpc, uint32_t *alert,uint32_t *mask)
{
...
static int wusb3812_get_power_status(struct tcpc_device *tcpc)
{
    int ret;
...
//#if IS_ENABLED(CONFIG_TCPC_VSAFE0V_DETECT_IC)
    ...
    ret = wusb3812_is_vsafe0v(tcpc);
    if (ret)
        tcpc->vbus_safe0v = true;
	else
	    tcpc->vbus_safe0v = false;
...
//kelong typec_is_attached_src û����
static int wusb3812_set_cc(struct tcpc_device *tcpc, int pull)
{
	...
	/* A06 code for AL7160A-3018 by xiongxiaoliang at 202400621 start */
	if ((tcpc->typec_state==typec_attached_src) &&
...
#if IS_ENABLED(CONFIG_TCPC_LOW_POWER_MODE)
...
// #if IS_ENABLED(CONFIG_TYPEC_CAP_NORP_SRC)
...
// 1956
#if IS_ENABLED(CONFIG_TCPC_WATCHDOG_EN)
...
#if IS_ENABLED(CONFIG_TCPC_INTRST_EN)
...
static struct tcpc_ops wusb3812_tcpc_ops = {
	...
	.get_alert_status_and_mask = wusb3812_get_alert_status,
...
#if CONFIG_USB_PD_RETRY_CRC_DISCARD
	.retransmit = wusb3812_retransmit,  //Kelong
...
static int wusb3812_i2c_probe(struct i2c_client *client)
{
	...
	/*A06_V code for SR-AL7160V-01-97 by yexuedong at 20240902 start*/
	/* set for tcpc_info */
	gxy_bat_set_tcpcinfo(GXY_BAT_TCPC_INFO_WUSB3812C);
	/*A06_V code for SR-AL7160V-01-97 by yexuedong at 20240902 end*/
#if TCPC_ENABLE_ANYMSG
...
static void wusb3812_i2c_remove(struct i2c_client *client)
{
