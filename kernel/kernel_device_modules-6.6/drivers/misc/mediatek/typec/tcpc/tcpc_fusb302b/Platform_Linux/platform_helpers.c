...
FSC_BOOL fusb_GPIO_Get_IntN(void)
{
	...
	else
	{
		...
		*/
		...
		{
			ret = !gpiod_get_value_cansleep(gpio_to_desc(chip->gpio_IntN));
		}
		else
		{
			ret = !gpiod_get_value(gpio_to_desc(chip->gpio_IntN)); // Int_N is active low
		}
		return (ret != 0);
	}
}
		...
		void *usr_ctx, void *app_ctx)
{
	...
	case CC2_ORIENT:
		...
		if (chip->port.ConnState == AudioAccessory) {
			...
			if (chip->port.sourceOrSink == SINK) {
				...
				if (chr_psy == NULL || IS_ERR(chr_psy)) {
					pr_info("chg_psy fail %s\n", __func__);
					return;
				}
				propval.intval = 2;
				...
				fusb_register_partner(chip);
				//ret = power_supply_set_property(chr_psy, POWER_SUPPLY_PROP_ONLINE,
				//		&propval);
				//if (ret < 0)
				//	pr_info("gaouxi11111 get chg_psy fail %s\n", __func__);
	...
	case CC_NO_ORIENT:
		...
		if (chip->usb_state != 0) {
			srcu_notifier_call_chain(&chip->pd_adapter->evt_nh, MTK_PD_CONNECT_NONE, NULL);
			/* hs14 code for AL6528ADEU-342 by wenyaqi at 2022/10/12 start */
			mtk_pd_noti = 1;
			/* hs14 code for AL6528ADEU-342 by wenyaqi at 2022/10/12 end */
		}
	...
	case PD_STATE_CHANGED:
		...
		if (chip->port.PolicyState == peSinkReady &&
			...
			if (mtk_pd_noti == 0) {
				...
				srcu_notifier_call_chain(&chip->pd_adapter->evt_nh, MTK_PD_CONNECT_PE_READY_SNK, NULL);
				mtk_pd_noti = 2;
			}
			/* hs14 code for AL6528ADEU-342 by wenyaqi at 2022/10/12 end */
		}
...
void fusb_init_mtk_pd_adapter(void)
{
	struct fusb30x_chip* chip = fusb30x_GetChip();
