	...
	case VBUS_LVL_5V:
		...
		if (chip->is_vbus_present != enable)
		{
			if (!chip->primary_charger) {
				chip->primary_charger = get_charger_by_name("primary_chg");
				if (!chip->primary_charger) {
			...
			if (enable == TRUE)
			{
				...
				charger_dev_enable_otg(chip->primary_charger, true);
				charger_dev_set_boost_current_limit(chip->primary_charger, 1500000);
				chip->is_vbus_present = TRUE;
			} else {
				pr_info("FUSB - %s, trying to disable vbus\n", __func__);
				charger_dev_enable_otg(chip->primary_charger, false);
