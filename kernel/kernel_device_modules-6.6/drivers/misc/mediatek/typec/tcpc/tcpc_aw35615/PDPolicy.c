        ...
        if (DPM_GetSinkCap(port->dpm, port)[i - 1].PDO.SupplyType == pdoTypeAugmented) {
            for (j = 0; j < port->SrcCapsHeaderReceived.NumDataObjects; j++) {
                if (port->SrcCapsReceived[j].PDO.SupplyType == pdoTypeAugmented) {
                    if ((port->SrcCapsReceived[j].PPSAPDO.MaxVoltage >=
                        DPM_GetSinkCap(port->dpm, port)[i - 1].PPSAPDO.MaxVoltage) &&
                        (port->SrcCapsReceived[j].PPSAPDO.MinVoltage <=
                        DPM_GetSinkCap(port->dpm, port)[i - 1].PPSAPDO.MinVoltage) &&
                        (port->SrcCapsReceived[j].PPSAPDO.MaxCurrent >=
                        DPM_GetSinkCap(port->dpm, port)[i - 1].PPSAPDO.MaxCurrent)) {
            ...
            for (j = 0; j < port->SrcCapsHeaderReceived.NumDataObjects; j++) {
                ...
                if (port->SrcCapsReceived[j].PDO.SupplyType == pdoTypeFixed) {
                    if (port->SrcCapsReceived[j].FPDOSupply.Voltage  ==
                        DPM_GetSinkCap(port->dpm, port)[i - 1].FPDOSink.Voltage) {
    ...
    if ((port->src_support_fixed == AW_TRUE) || (src_support_pps == AW_TRUE)) {
        ...
        if (src_support_pps == AW_TRUE) {
            ...
            port->SinkRequest.PPSRDO.Voltage =
                DPM_GetSinkCap(port->dpm, port)[i - 1].PPSAPDO.MinVoltage * 5;
            port->PpsEnabled = AW_TRUE;
        } else if (port->src_support_fixed == AW_TRUE) {
            /* Fixed request */
            /* Set the current based on the selected voltage (in 10mA units) */
            port->PpsEnabled = AW_FALSE;
            if (port->SrcCapsReceived[j].FPDOSupply.MaxCurrent >=
                DPM_GetSinkCap(port->dpm, port)[i - 1].FPDOSink.OperationalCurrent) {
            ...
            port->SinkRequest.FVRDO.MinMaxCurrent =
                port->SrcCapsReceived[j].FPDOSupply.MaxCurrent;
        }
