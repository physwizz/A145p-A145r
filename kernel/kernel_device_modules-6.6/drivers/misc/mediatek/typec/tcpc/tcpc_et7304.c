...
int et7304_get_alert_status(struct tcpc_device *tcpc, uint32_t *alert, uint32_t *mask)//, uint32_t *alert)
{
	struct et7304_chip *chip = tcpc_get_dev_data(tcpc);
	int ret;
	uint8_t buf[4] = {0};

	ret = et7304_block_read(chip->client, TCPC_V10_REG_ALERT, 4, buf);
	if (ret < 0)
		return ret;
	...
	*mask = le16_to_cpu(*(uint16_t *)&buf[2]);
	...
	ret = et7304_block_read(chip->client, ET7304_REG_ET_INT, 2, buf);
	if (ret < 0)
		return ret;
...
static int et_retransmit(struct tcpc_device *tcpc)
{
...
static struct tcpc_ops et7304_tcpc_ops = {
	...
	.get_alert_status_and_mask = et7304_get_alert_status,
				...
				//const struct i2c_device_id *id)
{
	...
	/* set for tcpc_info */
	#if IS_ENABLED(CONFIG_HQ_PROJECT_O22)
	gxy_bat_set_tcpcinfo(GXY_BAT_TCPC_INFO_ET7304);
	#endif
	/*A14 code for R-AL6528V-01-111 by chenyulin at 20240911 end*/
