...
/*A14_V code for AL6528AV-456 by yuli at 20250421 start*/
#if IS_ENABLED(CONFIG_DRM_MEDIATEK_V2)
static int mtk_drm_notifier_callback(struct notifier_block* self,
                                        unsigned long event, void* data)
{
    int blank = *(int *)data;
    int retval = 0;

    CDFINGER_DBG("DRM event:%lu,blank:%d", event, blank);
    if (event != MTK_DISP_EVENT_BLANK /* MTK_DISP_EVENT_BLANK */) {
        return 0;
    }

    switch (blank) {
        case MTK_DISP_BLANK_UNBLANK:
            CDFINGER_DBG("SCREEN_ON");
...
/*A14_V code for AL6528AV-456 by yuli at 20250421 start*/
#if IS_ENABLED(CONFIG_DRM_MEDIATEK_V2)
    cdfinger->notifier.notifier_call = mtk_drm_notifier_callback;
    if (mtk_disp_notifier_register("cdfinger Fingerprint", &cdfinger->notifier)) {
        CDFINGER_ERR("%s: Unable to register mtk_drm_notif\n", __func__);
    }
#else//CONFIG_FB
