...
/*A06_V code for AL7160AV-521 by yuli at 20250421 start*/
#if IS_ENABLED(CONFIG_DRM_MEDIATEK_V2)
static int  anc_mtk_drm_callback(struct notifier_block *notif, unsigned long value, void *v)
{
    int retval = 0;
    int blank = *(int *)v;
    char netlink_msg = (char)ANC_NETLINK_EVENT_INVALID;
...
/*A06_V code for AL7160AV-521 by yuli at 20250421 start*/
#if IS_ENABLED(CONFIG_DRM_MEDIATEK_V2)
    ANC_LOGD("register mtk drm callback");
    p_data->notif.notifier_call = anc_mtk_drm_callback;
    ret_val = mtk_disp_notifier_register("ANC Fingerprint",&p_data->notif);
    if (ret_val) {
            ANC_LOGE("Unable to register mtk_disp_notifier_register,ret_val=%d", ret_val);
            goto out;
    }
#else//CONFIG_FB
