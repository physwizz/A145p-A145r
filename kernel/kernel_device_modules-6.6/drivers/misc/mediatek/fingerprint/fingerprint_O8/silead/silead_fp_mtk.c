...
static void silfp_hw_poweron(struct silfp_data *fp_dev)
{
    ...
    if (fp_dev->pin.fp_spi_mode) {
        mdelay(1);
        pinctrl_select_state(fp_dev->pin.pinctrl, fp_dev->pin.fp_spi_mode);
    }
...
#ifdef BSP_SIL_POWER_SUPPLY_PINCTRL
    /*A06 code for SR-AL7160A-01-16 by hehaoran at 20240422 start*/
    if (fp_dev->pin.pins_rst_l) {
        pinctrl_select_state(fp_dev->pin.pinctrl, fp_dev->pin.pins_rst_l);
        mdelay(1);
    }

    if (fp_dev->pin.fp_gpio_mode) {
        pinctrl_select_state(fp_dev->pin.pinctrl, fp_dev->pin.fp_gpio_mode);
        mdelay(1);
    }
    /*A06 code for SR-AL7160A-01-16 by hehaoran at 20240422 end*/
...
/*A06 code for SR-AL7160A-01-16 by hehaoran at 20240422 start*/
    ...
    /*A06 code for AL7160A-3016 by hehaoran at 20240613 start*/
    /* Power control by GPIOs */
    fp_dev->pin.fp_gpio_mode = NULL;
    fp_dev->pin.fp_spi_mode = NULL;
    fp_dev->pin.pins_avdd_h = NULL;
    ...
    /*A06 code for AL7160A-3016 by hehaoran at 20240613 end*/
#endif /* BSP_SIL_POWER_SUPPLY_PINCTRL */
/*A06 code for SR-AL7160A-01-16 by hehaoran at 20240422 end*/
#ifdef BSP_SIL_POWER_SUPPLY_GPIO
    if ( fp_dev->avdd_port >= 0 ) {
...
static void silfp_hw_reset(struct silfp_data *fp_dev, u8 delay)
{
    ...
    if (fp_dev->pin.pinctrl == NULL) {
        LOG_MSG_DEBUG(ERR_LOG, "[%s] pinctrl is NULL\n", __func__);
        return;
    }
    if (fp_dev->pin.pins_rst_l) {
        pinctrl_select_state(fp_dev->pin.pinctrl, fp_dev->pin.pins_rst_l);
...
/*A06 code for SR-AL7160A-01-16 by hehaoran at 20240422 start*/
    ...
    fp_dev->pin.fp_spi_mode = pinctrl_lookup_state(fp_dev->pin.pinctrl, "fp_spi_mode");
    if (IS_ERR(fp_dev->pin.fp_spi_mode)) {
        ret = PTR_ERR(fp_dev->pin.fp_spi_mode);
        pr_info("%s can't find silfp fp_spi_mode\n", __func__);
        return ret;
    }

    fp_dev->pin.fp_gpio_mode = pinctrl_lookup_state(fp_dev->pin.pinctrl, "fp_gpio_mode");
    if (IS_ERR(fp_dev->pin.fp_gpio_mode)) {
        ret = PTR_ERR(fp_dev->pin.fp_gpio_mode);
        pr_info("%s can't find silfp fp_gpio_mode\n", __func__);
        return ret;
    }
...
err_irq:
    //gpio_free(fp_dev->int_port);
    /*A06 code for AL7160A-1940 by hehaoran at 20240531 start*/
    silfp_power_deinit(fp_dev);
    /*A06 code for AL7160A-1940 by hehaoran at 20240531 end*/
//err:
    fp_dev->int_port = -1;
    fp_dev->rst_port = -1;
