...
/*hs14 code for SR-AL6528A-01-265 by xiongxiaoliang at 2022/10/10 start*/
...
/*hs14 code for SR-AL6528A-01-265 by xiongxiaoliang at 2022/10/10 end*/
...
{
    /*hs14 code for SR-AL6528A-01-271 by zhangziyi at 2022/10/12 start*/
    struct silfp_msg_list *list = NULL;
    struct silfp_msg_list *next = NULL;
    /*hs14 code for SR-AL6528A-01-271 by zhangziyi at 2022/10/12 end*/
    unsigned long flags;
...
static int  silfp_mtk_drm_callback(struct notifier_block *notif, unsigned long value, void *v)
{
    struct silfp_data *fp_dev = container_of(notif, struct silfp_data, notif);
    int blank = *(int *)v;
    int retval = 0;
    /* If we aren't interested in this event, skip it immediately ... */
    if (value != SIL_EVENT_BLANK /* MTK_DISP_EARLY_EVENT_BLANK */) {
        return 0;
    }
...
/*A06_V code for AL7160AV-521 by yuli at 20250421 start*/
#if IS_ENABLED(CONFIG_DRM_MEDIATEK_V2)
    fp_dev->notif.notifier_call = silfp_mtk_drm_callback;
#else//CONFIG_FB
...
silfp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case SIFP_IOC_INIT:
        ...
        if (err < 0) {
            LOG_MSG_DEBUG(INFO_LOG,"[%s]silfp_resource_init fail\n",__func__);
            retval = -EFAULT;
        }
...
static void silfp_shutdown(struct spi_device *spi)
{
    struct silfp_data	*fp_dev = spi_get_drvdata(spi);
    silfp_resource_deinit(fp_dev);
}
