...
/*A06_V code for AL7160AV-521 by yuli at 20250421 start*/
#if IS_ENABLED(CONFIG_DRM_MEDIATEK_V2)
#include "mtk_disp_notify.h"
#endif//CONFIG_DRM_MEDIATEK_V2
/*A06_V code for AL7160AV-521 by yuli at 20250421 end*/
#ifdef CONFIG_COMPAT
...
/*A06 code for SR-AL7160A-01-16|AL7160A-2966 by hehaoran at 20240620 start*/
    ...
    if (ff_ctx->b_power_on ^ on) {
        ...
        if (!ff_ctx->b_use_regulator) {
            if (ff_ctx->b_use_pinctrl && ff_ctx->pinctrl) {
                ...
                if (on && ff_ctx->pins_power_high && ff_ctx->fp_spi_mode ) {
                    ret = pinctrl_select_state(ff_ctx->pinctrl, ff_ctx->pins_power_high);
                    mdelay(1);
                    retval = pinctrl_select_state(ff_ctx->pinctrl, ff_ctx->fp_spi_mode);
                    if (retval < 0){
                        FF_LOGE("set fp_spi_mode failed,ret=%d", retval);
                    }
                } else if (!on && ff_ctx->pins_power_low && ff_ctx->fp_gpio_mode) {
                    retval = pinctrl_select_state(ff_ctx->pinctrl, ff_ctx->fp_gpio_mode);
                    if (retval < 0){
                        FF_LOGE("set fp_gpio_mode failed,ret=%d", retval);
                    } else {
                        mdelay(1);
                    }
                    ret = pinctrl_select_state(ff_ctx->pinctrl, ff_ctx->pins_power_low);
                } else {
                    FF_LOGE("pinctrl point has NULL\n");
                }
            }
...
/*A06 code for SR-AL7160A-01-16|AL7160A-2966 by hehaoran at 20240620 end*/
...
/*A06_V code for AL7160AV-521 by yuli at 20250421 start*/
#if IS_ENABLED(CONFIG_DRM_MEDIATEK_V2)
static int mtk_drm_notifier_callback(struct notifier_block *notif, unsigned long value, void *v)
{
    int blank = *(int *)v;
    ff_context_t *ff_ctx = container_of(notif, ff_context_t, mtk_drm_notif);
    char *uevent_env[2];
    bool this_screen_on;
    ...
    if (value != MTK_DISP_EVENT_BLANK) {
        FF_LOGI("value(%lu) do not need process\n", value);
        return NOTIFY_DONE;
    }

    FF_LOGD("DRM value:%lu,blank:%d", value, blank);
    switch (blank) {
...
/*A06_V code for AL7160AV-521 by yuli at 20250421 start*/
#if IS_ENABLED(CONFIG_DRM_MEDIATEK_V2)
        ff_ctx->mtk_drm_notif.notifier_call = mtk_drm_notifier_callback;
        ret = mtk_disp_notifier_register("Focal Fingerprint", &ff_ctx->mtk_drm_notif);
        if (ret) {
            FF_LOGE("Unable to register mtk_drm_notif,ret=%d", ret);
        }
#endif//CONFIG_DRM_MEDIATEK_V2
/*A06_V code for AL7160AV-521 by yuli at 20250421 end*/
    }
...
err_init_power:
    if (ff_ctx->b_use_pinctrl && ff_ctx->pinctrl) {
        ...
        /*A06 code for AL7160A-3474 by hehaoran at 20240620 start*/
        ff_ctx->fp_gpio_mode = NULL;
        ff_ctx->fp_spi_mode = NULL;
        /*A06 code for AL7160A-3474 by hehaoran at 20240620 end*/
...
static int ff_ctl_free_driver(ff_context_t *ff_ctx)
{
    ...
    FF_LOGD("'%s' enter.", __func__);
    /*A06 code for AL7160A-3474 by hehaoran at 20240620 start*/
    mutex_lock(&ff_ctx->free_lock);
    FF_LOGI("%s has in mutex\n",__func__);
    if ((!ff_ctx) || !ff_ctx->b_driver_inited) {
        FF_LOGE("ff_ctx is null, or driver has not initialized");
        mutex_unlock(&ff_ctx->free_lock);
        return -EINVAL;
    }
    /*A06 code for AL7160A-3474 by hehaoran at 20240620 end*/
    /* Disable SPI clock. */
...
static int ff_init_pins(ff_context_t *ff_ctx)
{
    ...
    if (ff_ctx->b_use_pinctrl) {
        ...
        ff_ctx->fp_gpio_mode = pinctrl_lookup_state(ff_ctx->pinctrl, "fp_gpio_mode");
        if (IS_ERR_OR_NULL(ff_ctx->fp_gpio_mode)) {
            ret = PTR_ERR(ff_ctx->fp_gpio_mode);
            FF_LOGE("ff_pins_fp_gpio_mode not found,ret=%d", ret);
            devm_pinctrl_put(ff_ctx->pinctrl);
            ff_ctx->pins_reset_low = NULL;
            ff_ctx->pins_reset_high = NULL;
            ff_ctx->pins_irq_as_int = NULL;
        ...
        ff_ctx->fp_spi_mode = pinctrl_lookup_state(ff_ctx->pinctrl, "fp_spi_mode");
        if (IS_ERR_OR_NULL(ff_ctx->fp_spi_mode)) {
            ret = PTR_ERR(ff_ctx->fp_spi_mode);
            FF_LOGE("ff_pins_fp_spi_mode not found,ret=%d", ret);
            devm_pinctrl_put(ff_ctx->pinctrl);
            ff_ctx->pins_reset_low = NULL;
            ff_ctx->pins_reset_high = NULL;
            ff_ctx->pins_irq_as_int = NULL;
        ...
        /*set fp_gpio_mode as default*/
        ret = pinctrl_select_state(ff_ctx->pinctrl, ff_ctx->fp_gpio_mode);
        if (ret < 0) {
            FF_LOGE("pinctrl_select_state:fp_gpio_mode fails");
            devm_pinctrl_put(ff_ctx->pinctrl);
            ff_ctx->pins_reset_low = NULL;
            ff_ctx->pins_reset_high = NULL;
            ff_ctx->pins_irq_as_int = NULL;
...
/*A06 code for SR-AL7160A-01-16 by hehaoran at 20240422 start*/
static void ff_shutdown(struct platform_device *pdev)
{
    int ret = 0;
    ff_context_t *ff_ctx = platform_get_drvdata(pdev);
    FF_LOGI("'%s' enter.", __func__);
    ret = ff_ctl_free_driver(ff_ctx);
    if (ret < 0) {
        FF_LOGE("ff_shutdown free driver fails");
    }
    FF_LOGI("'%s' leave.", __func__);
}
/*A06 code for SR-AL7160A-01-16 by hehaoran at 20240422 end*/
