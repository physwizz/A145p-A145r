 ...
 */
#define PFX "CAM_CAL_a0603ddfrontmt811"
...
struct i2c_client *hqextend_g_pstI2CclientG;

struct a0603ddfrontmt811_otp_struct
{
...
static int a0603ddfrontmt811_read_otp(struct i2c_client *client,
				u32 start_addr, u8 *data, u32 otp_len)
{
	...
	printk("a0603ddfrontmt811_read_otp start_addr = %x size = %d data read = %d\n", start_addr, otp_len, rtn);
...
unsigned int a0603ddfrontmt811_read_region(struct i2c_client *client, unsigned int addr,
				unsigned char *data, unsigned int size)
{
	/* otp_offset need update when otp data write offset confirm */
	//u16 otp_offset = 0x0604;
	//addr += otp_offset;
        hqextend_g_pstI2CclientG = client;
	if (a0603ddfrontmt811_read_otp(client, addr, data, size) == 0)
		return size;
	else
		return 0;
}
