 ...
 */
#define PFX "CAM_CAL_a0606ddfrontmt811"
...
struct i2c_client *hqextend_g_pstI2CclientGFPC;

struct a0606ddfrontmt811_otp_struct
{
...
static int sensor_reg_read(struct i2c_client *client, const u16 addr)
{
	...
	if (ret < 0) {
		pr_warn("reading register %x from %x failed\n", addr, client->addr);
		return ret;
	}
...
static int sensor_reg_write (struct i2c_client *client, const u16 addr, const u8 data)
{
	...
	if (ret == 1)
		...
		pr_warn("write register %x from %x failed\n", addr, client->addr);
		return -EIO;
	}
}

static int a0606ddfrontmt811_read_otp(struct i2c_client *client,
				u32 start_addr, u8 *data, u32 otp_len)
{
	...
	printk("a0606ddfrontmt811_read_otp start_addr = %x size = %d data read = %d\n", start_addr, otp_len, rtn);

	if (otp_len > 8192) {
		pr_warn("otp read len can not over 8K");
		rtn = -1;
		return rtn;
	}
	do {
...
unsigned int a0606ddfrontmt811_read_region(struct i2c_client *client, unsigned int addr,
				unsigned char *data, unsigned int size)
{
	/* otp_offset need update when otp data write offset confirm */
	//u16 otp_offset = 0x0604;
	//addr += otp_offset;
        hqextend_g_pstI2CclientGFPC = client;
	if (a0606ddfrontmt811_read_otp(client, addr, data, size) == 0)
		return size;
	else
		return 0;
}
