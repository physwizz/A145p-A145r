...
#include <linux/ssdev_in_mtk/usb/typec/manager/usb_typec_manager_notifier.h>
...
static int manager_notifier_init(void)
{
	...
	manager_device = (struct device *) sec_device_create(NULL, "typec_manager");
...
#if IS_ENABLED(CONFIG_HICCUP_CHARGER) && IS_ENABLED(CONFIG_BATTERY_SAMSUNG)
...
#if IS_ENABLED(CONFIG_PDIC_ALTERNATE_MODE)
...
MODULE_AUTHOR("Samsung USB Type C");
MODULE_DESCRIPTION("USB Type C Notifier");
MODULE_LICENSE("GPL");
