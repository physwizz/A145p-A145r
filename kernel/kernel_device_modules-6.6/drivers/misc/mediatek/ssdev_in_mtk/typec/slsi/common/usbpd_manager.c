...
#endif
#include <linux/ssdev_in_mtk/usb/typec/common/pdic_core.h>
...
void usbpd_manager_plug_attach(struct device *dev, muic_attached_dev_t new_dev)
{
	...
	if (new_dev == ATTACHED_DEV_TYPE3_CHARGER_MUIC) {
		...
		pdic_notifier_notify((PD_NOTI_TYPEDEF *)&pd_notifier, &pd_noti, 1/* pdic_attach */);
#endif
	}
...
#if IS_ENABLED(CONFIG_USB_HW_PARAM)
				if (o_notify)
					inc_hw_param(o_notify, USB_PDIC_VR_USE_COUNT);
#endif
				break;
...
#if IS_ENABLED(CONFIG_USB_HW_PARAM)
				if (o_notify)
					inc_hw_param(o_notify, USB_PDIC_DEX_USE_COUNT);
#endif
				break;
...
static int of_usbpd_manager_dt(struct usbpd_manager_data *_data)
{
	...
	if (np == NULL) {
		...
		ret = of_property_read_u32(np, "pdic,max-power",
				&_data->max_power);
		if (ret < 0)
			pr_err("%s error reading max_power %d\n",
					__func__, _data->max_power);

		ret = of_property_read_u32(np, "pdic,op-power",
				&_data->op_power);
		if (ret < 0)
			pr_err("%s error reading op_power %d\n",
					__func__, _data->max_power);

		ret = of_property_read_u32(np, "pdic,max-current",
				&_data->max_current);
		if (ret < 0)
			pr_err("%s error reading max_current %d\n",
					__func__, _data->max_current);

		ret = of_property_read_u32(np, "pdic,min-current",
				&_data->min_current);
		if (ret < 0)
		...
		_data->usb_com_capable = of_property_read_bool(np,
						     "pdic,usb-com-capable");
		_data->no_usb_suspend = of_property_read_bool(np,
						     "pdic,no-usb-suspend");

		/* source capability */
		ret = of_property_read_u32(np, "source,max-voltage",
				&_data->source_max_volt);
		ret = of_property_read_u32(np, "source,min-voltage",
				&_data->source_min_volt);
		ret = of_property_read_u32(np, "source,max-power",
				&_data->source_max_power);

		/* sink capability */
		ret = of_property_read_u32(np, "sink,capable-max-voltage",
