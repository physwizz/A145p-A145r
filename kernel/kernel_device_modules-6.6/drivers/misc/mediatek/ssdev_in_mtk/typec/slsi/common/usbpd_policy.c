...
policy_state usbpd_policy_src_startup(struct policy_data *policy)
{
	...
	**********************************************/
	...
	pd_data->phy_ops.set_pd_control(pd_data, USBPD_PD_ON);
...
policy_state usbpd_policy_src_ready(struct policy_data *policy)
{
	...
	**********************************************/
	...
	/* OP_Mode Clear */
	//pd_data->phy_ops.op_mode_clear(pd_data);
	...
	CHECK_CMD(pd_data, MANAGER_REQ_SRPDAP_CHANGE, PE_SRC_Send_Capabilities);
...
policy_state usbpd_policy_src_hard_reset(struct policy_data *policy)
{
	...
	**********************************************/
	...
	/* Configuration Channel On */
	pd_data->phy_ops.set_pd_control(pd_data, USBPD_PD_OFF);
...
policy_state usbpd_policy_snk_ready(struct policy_data *policy)
{
	...
	**********************************************/
	...
	pd_data->phy_ops.pd_instead_of_vbus(pd_data, false);
...
policy_state usbpd_policy_ufp_vdm_send_identity(struct policy_data *policy)
{
	...
	**********************************************/
	...
	specification_revision = pd_data->protocol_rx.msg_header.spec_revision;
	...
	if(specification_revision >= USBPD_PD3_0)
...
policy_state usbpd_policy_frs_src_snk_evaluate_swap(struct policy_data *policy)
{
	...
	/**********************************************
	Actions on entry:
	Ask Device Policy Manager if Fast Role Swap signaled on PD wire
	**********************************************/
