...
#include <linux/ssdev_in_mtk/mfd/samsung/s2mu106.h>
#include <linux/err.h>

/* TODO : add IP Header file include*/
#if IS_ENABLED(CONFIG_CHARGER_S2MU106)
#include "../../../battery/charger/s2mu106/s2mu106_charger.h"
#endif
...
{
	struct s2mu106_dev *p_s2mu106 = irq_get_chip_data(data->irq);

    mutex_lock(&p_s2mu106->irqlock);
}
...
static void s2mu106_irq_sync_unlock(struct irq_data *data)
{
	...
	int i;
	u8 mask_reg;
	struct i2c_client *i2c = p_s2mu106->i2c;

	for (i = 0; i < S2MU106_IRQ_GROUP_NR; i++) {
		mask_reg = s2mu106_mask_reg[i];
		i2c = get_i2c(p_s2mu106, i);
		if (mask_reg == S2MU106_REG_INVALID ||
				IS_ERR_OR_NULL(i2c))
			continue;
		p_s2mu106->irq_masks_cache[i] = p_s2mu106->irq_masks_cur[i];

		s2mu106_write_reg(i2c, s2mu106_mask_reg[i],
				p_s2mu106->irq_masks_cur[i]);
	}

    mutex_unlock(&p_s2mu106->irqlock);
}

static const inline struct s2mu106_irq_data *
irq_to_s2mu106_irq(struct s2mu106_dev *p_s2mu106, int irq)
{
	return &s2mu106_irqs[irq - p_s2mu106->irq_base];
}
...
static void s2mu106_irq_mask(struct irq_data *data)
{
	...
	const struct s2mu106_irq_data *irq_data =
	    irq_to_s2mu106_irq(p_s2mu106, data->irq);

	if (irq_data->group >= S2MU106_IRQ_GROUP_NR)
		return;

	p_s2mu106->irq_masks_cur[irq_data->group] |= irq_data->mask;
}
...
{
	struct s2mu106_dev *p_s2mu106 = data;
	u8 irq_reg[S2MU106_IRQ_GROUP_NR] = {0, };
	int i, ret;
	u8 irq_src;

	ret = s2mu106_read_reg(p_s2mu106->i2c, S2MU106_REG_IPINT, &irq_src);
	if (ret) {
	...
	if (irq_src & S2MU106_IRQSRC_CHG) {
		ret = s2mu106_bulk_read(p_s2mu106->i2c, S2MU106_CHG_INT1,
	...
	if (irq_src & S2MU106_IRQSRC_FLED) {
		ret = s2mu106_bulk_read(p_s2mu106->i2c, S2MU106_FLED_INT1,
	...
	if (irq_src & S2MU106_IRQSRC_AFC) {
		s2mu106_read_reg(p_s2mu106->muic, S2MU106_REG_AFC_INT, &irq_reg[AFC_INT]);
		pr_info("%s: AFC interrupt(0x%02x)\n", __func__, irq_reg[AFC_INT]);
	}
#endif
#if IS_ENABLED(CONFIG_MUIC_S2MU106)
	if (irq_src & S2MU106_IRQSRC_MUIC) {
		s2mu106_read_reg(p_s2mu106->muic, S2MU106_REG_MUIC_INT1, &irq_reg[MUIC_INT1]);
		s2mu106_read_reg(p_s2mu106->muic, S2MU106_REG_MUIC_INT2, &irq_reg[MUIC_INT2]);
	...
	if (irq_src & S2MU106_IRQSRC_PM) {
		s2mu106_read_reg(p_s2mu106->muic, S2MU106_PM_VALUP1, &irq_reg[PM_VALUP1]);
		s2mu106_read_reg(p_s2mu106->muic, S2MU106_PM_VALUP2, &irq_reg[PM_VALUP2]);
		s2mu106_read_reg(p_s2mu106->muic, S2MU106_PM_INT1, &irq_reg[PM_INT1]);
		s2mu106_read_reg(p_s2mu106->muic, S2MU106_PM_INT2, &irq_reg[PM_INT2]);
	...
	if (irq_src & S2MU106_IRQSRC_HAPTIC) {
        	s2mu106_read_reg(p_s2mu106->haptic, S2MU106_REG_HAPTIC_INT, &irq_reg[HAPTIC_INT]);
...
int s2mu106_irq_init(struct s2mu106_dev *s2mu106)
{
	...
	pr_err("%s:%s group2 irq\n", MFD_DEV_NAME, __func__);

	for (i = 0; i < S2MU106_IRQ_NR; i++) {
		cur_irq = 0;
		cur_irq = i + s2mu106->irq_base;
		pr_err("%s:%s irq nr : %d cur_irq : %d\n", MFD_DEV_NAME, __func__, i, cur_irq);
	...
	pr_err("%s: %s init top-irq mask(0x%02x)\n", MFD_DEV_NAME, __func__, i2c_data);
