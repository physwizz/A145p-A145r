...
#include <linux/ssdev_in_mtk/host_notify.h>
#endif
...
#endif
module_param(__pdic_info, int, 0);
...
static int muic_manager_get_ext_property(struct power_supply *psy,
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_EXT_PROP_MIN ... POWER_SUPPLY_EXT_PROP_MAX:
		muic_manager_get_ext_property(psy, ext_psp, val);
		break;
	default:
		return -EINVAL;
	}
...
static int muic_manager_set_ext_property(struct power_supply *psy,
		enum power_supply_ext_property ext_psp,
		...
		const union power_supply_propval *val)
{
	...
	switch (psp) {
		default:
			return muic_manager_set_ext_property(psy, ext_psp, val);
	}
	return 0;
}
