...
#include "a0603ddfrontmt811_mipi_raw.h"

#define PFX "a0603ddfrontmt811_mipi_raw"
...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = A0603DDFRONTMT811_SENSOR_ID,
        ...
        do
        {
            ...
            *sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
            if (*sensor_id == imgsensor_info.sensor_id)
            {
                ...
                if(mt811VendorId != 0x00 && mt811VendorId != 0x83){
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
    ...
    if (enable)
    {
        ...
        write_cmos_sensor(0x090b, 0x00);
        write_cmos_sensor(0x0902, 0xc0);
        write_cmos_sensor(0x0903, 0x0c);
        write_cmos_sensor(0x0904, 0x90);
        write_cmos_sensor(0x0905, 0x09);
        write_cmos_sensor(0x0906, 0x80);
        write_cmos_sensor(0x0907, 0x05);
        write_cmos_sensor(0x0908, 0x78);
        write_cmos_sensor(0x0909, 0x00);
        write_cmos_sensor(0x0910, 0x00);
        write_cmos_sensor(0x0911, 0x00);
        write_cmos_sensor(0x0912, 0x00);
        write_cmos_sensor(0x0913, 0x00);
        write_cmos_sensor(0x0914, 0x00);
        write_cmos_sensor(0x0915, 0x00);
        write_cmos_sensor(0x0916, 0x00);
        write_cmos_sensor(0x0917, 0x00);
        write_cmos_sensor(0x090a, 0x00);
...
/* A06 code for SR-AL7160A-01-537 by dingling at 20240719 start */
static kal_uint32 streaming_control(kal_bool enable)
{
    kal_uint8 stream_status = 0;
    kal_uint8 cnt = 105;
    pr_info("streaming_enable begin(0=Sw tandby,1=streaming): %d\n", enable);
    if(enable)
    {
        write_cmos_sensor(0x0600, 0X01);
        msleep(1);
    ...
    else
    {
        ...
        do {
                stream_status = read_cmos_sensor(0x0410);
                if ((stream_status & 0x04) == 0x04) {
                    break;
                } else {
                            mdelay(1);
                            cnt--;
    ...
    pr_info("streaming_enable end (0=Sw tandby,1=streaming): %d\n", enable);
    return ERROR_NONE;
}
/* A06 code for SR-AL7160A-01-537 by dingling at 20240719 end */
...
UINT32 A0603DDFRONTMT811_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT  **pfFunc)
{
    LOG_INF("A0603DDFRONTMT811_MIPI_RAW_SensorInit\n");
    if (pfFunc!=NULL)
            *pfFunc=&sensor_func;
    return ERROR_NONE;
}
