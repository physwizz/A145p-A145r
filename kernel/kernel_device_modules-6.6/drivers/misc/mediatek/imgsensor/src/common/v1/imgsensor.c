...
extern char hq_camerachipSN[255];
extern char *hq_cameraSNModuleInfo[3];
#endif
...
/* A06 code for SR-AL7160V-224 by wangtao at 20241015 start */
    ...
    {
        if ((!strcmp(psensor_inst->psensor_name,"a0601lybacks5kjn1_mipi_raw")) ||
            (!strcmp(psensor_inst->psensor_name,"a0602xlbacks5kjn1_mipi_raw")) ||
            (!strcmp(psensor_inst->psensor_name,"a0603txdbacks5kjn1_mipi_raw")) ||
            (!strcmp(psensor_inst->psensor_name,"a0604xlbacks5kjn1_mipi_raw")) ||
            (!strcmp(psensor_inst->psensor_name,"a0605ofbacks5kjn1_mipi_raw")))
        {
            hq_cameraSNModuleInfo[0] = psensor_inst->psensor_name;
            if (err == IMGSENSOR_RETURN_SUCCESS)
            {
                MUINT32 hq_cameraSN[15] ={0};
                imgsensor_sensor_feature_control(
                    psensor,
                    SENSOR_FEATURE_GET_CAMERA_CHIP_ID,
                    (MUINT8 *)hq_cameraSN,
                    &retLen);
                sprintf(hq_camerachipSN, "%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c", hq_cameraSN[0], hq_cameraSN[1], hq_cameraSN[2]
                , hq_cameraSN[3], hq_cameraSN[4], hq_cameraSN[5], hq_cameraSN[6], hq_cameraSN[7], hq_cameraSN[8], hq_cameraSN[9], hq_cameraSN[10]
            , hq_cameraSN[11], hq_cameraSN[12], hq_cameraSN[13], hq_cameraSN[14]);
                pr_err("[O8 read SN number]=%s",hq_camerachipSN);
            }
        }
    }
#endif
