...
#define mtktsusb_TEMP_CRIT 60000	/* 60.000 degree Celsius */
...
#define mtktsusb_printk(fmt, args...) \
pr_info("[Thermal/tzusb]" fmt, ##args)
...
static __s32 mtktsusb_thermistor_conver_temp(__s32 Res)
{
	int i = 0;
	int asize = 0;
	__s32 RES1 = 0, RES2 = 0;
	...
	if (Res >= USB_Temperature_Table[0].TemperatureR) {
		...
		TAP_Value = TAP_Value * 1000;
#endif
	} else if (Res <= USB_Temperature_Table[asize - 1].TemperatureR) {
		TAP_Value = 125;	/* max */
#ifdef APPLY_PRECISE_BTS_TEMP
		TAP_Value = TAP_Value * 1000;
#endif
		...
		for (i = 0; i < asize; i++) {
			if (Res >= USB_Temperature_Table[i].TemperatureR) {
				RES2 = USB_Temperature_Table[i]
					.TemperatureR;

				TMP2 = USB_Temperature_Table[i]
					.USB_Temp ;
...
static __s32 mtk_ts_usb_volt_to_temp(__u32 dwVolt)
{
	__s32 TRes;
	__u64 dwVCriAP = 0;
	__u64 dwVCriAP2 = 0;
	...
	/* convert register to temperature */
	USB_TMP  = mtktsusb_thermistor_conver_temp(TRes);

	return USB_TMP ;
}

static int get_hw_tsusb_temp(void)
{
#if IS_ENABLED(CONFIG_DEVICE_MODULES_MEDIATEK_MT6577_AUXADC)
	int val = 0;
	int ret = 0, output;
	...
	if (IS_ERR_OR_NULL(thermistor_ch3)) {
		mtktsusb_dprintk("invalid thermistor_ch3:0x%p\n", thermistor_ch3);
		return ret;
	}
	ret = iio_read_channel_processed(thermistor_ch3, &val);
	mtktsusb_dprintk("%s val=%d\n", __func__, val);

	if (ret < 0) {
		mtktsusb_printk("IIO channel read failed %d\n", ret);
		return ret;
	}
	...
	mtktsusb_dprintk("APtery output mV = %d\n", ret);
	output = mtk_ts_usb_volt_to_temp(ret);
	mtktsusb_dprintk("BTSUSB output temperature = %d\n", output);
	return output;
}

static DEFINE_MUTEX(BTSUSB_lock);
/*int ts_mtktsusb_at_boot_time = 0;*/
int mtktsusb_get_hw_temp(void)
{
	int t_ret = 0;

	mutex_lock(&BTSUSB_lock);

	/* get HW AP temp (TSAP) */
	/* cat /sys/class/power_supply/AP/AP_temp */
	t_ret = get_hw_tsusb_temp();
#ifndef APPLY_PRECISE_BTS_TEMP
	t_ret = t_ret * 1000;
#endif

	mutex_unlock(&BTSUSB_lock);

	if (t_ret > 40000)	/* abnormal high temp */
		mtktsusb_printk("T_tsusb=%d\n", t_ret);

	mtktsusb_dprintk("[%s] T_tsusb, %d\n", __func__,
									t_ret);
	return t_ret;
}
...
{
	*t = mtktsusb_get_hw_temp();

	if ((int)*t > 52000)
		mtktsusb_dprintk("T=%d\n", (int)*t);
...
static int mtktsusb_bind(struct thermal_zone_device *thermal,
			      struct thermal_cooling_device *cdev)
{
	...
	mtktsusb_dprintk("[%s] binding OK, %d\n", __func__,
								table_val);
	return 0;
}
...
static int mtktsusb_change_mode(struct thermal_zone_device *thermal,
...
static struct thermal_zone_device_ops mtktsusb_dev_ops = {
	...
	.change_mode = mtktsusb_change_mode,
...
struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	...
	if (sscanf
	    (ptr_mtktsusb_data ->desc,
	     "%d %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d",
		&num_trip,
		&ptr_mtktsusb_data ->trip[0], &ptr_mtktsusb_data ->t_type[0],
		ptr_mtktsusb_data ->bind0,
		&ptr_mtktsusb_data ->trip[1], &ptr_mtktsusb_data ->t_type[1],
		ptr_mtktsusb_data ->bind1,
		&ptr_mtktsusb_data ->trip[2], &ptr_mtktsusb_data ->t_type[2],
		ptr_mtktsusb_data ->bind2,
		&ptr_mtktsusb_data ->trip[3], &ptr_mtktsusb_data ->t_type[3],
		ptr_mtktsusb_data ->bind3,
		&ptr_mtktsusb_data ->trip[4], &ptr_mtktsusb_data ->t_type[4],
		ptr_mtktsusb_data ->bind4,
		&ptr_mtktsusb_data ->trip[5], &ptr_mtktsusb_data ->t_type[5],
		ptr_mtktsusb_data ->bind5,
		&ptr_mtktsusb_data ->trip[6], &ptr_mtktsusb_data ->t_type[6],
		ptr_mtktsusb_data ->bind6,
		&ptr_mtktsusb_data ->trip[7], &ptr_mtktsusb_data ->t_type[7],
		ptr_mtktsusb_data ->bind7,
		&ptr_mtktsusb_data ->trip[8], &ptr_mtktsusb_data ->t_type[8],
		ptr_mtktsusb_data ->bind8,
		&ptr_mtktsusb_data ->trip[9], &ptr_mtktsusb_data ->t_type[9],
		ptr_mtktsusb_data ->bind9,
		&ptr_mtktsusb_data ->time_msec) == 32) {

		down(&sem_mutex);
		mtktsusb_dprintk(
			"[%s] mtktsusb_unregister_thermal\n", __func__);
	...
	aee_kernel_warning_api(__FILE__, __LINE__, DB_OPT_DEFAULT,
					"mtktsusb_write", "Bad argument");
	#endif
	kfree(ptr_mtktsusb_data );
	return -EINVAL;
}

void mtktsusb_prepare_table(int table_num)
{
	...
	pr_notice("[Thermal/tzusb] %s table_num=%d\n",
						__func__, table_num);

}

static int mtktsusb_param_read(struct seq_file *m, void *v)
{
	seq_printf(m, "%d\n", g_RAP_pull_up_R);
...
static ssize_t mtktsusb_param_write(
struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	int len = 0;
	struct param_data {
	...
	if (sscanf
		...
		if (!strcmp(ptr_param_data->pull_R, "PUP_R")) {
			...
			kfree(ptr_param_data);
			mtktsusb_printk(
					"[mtktsusb_write] bad PUP_R argument\n");
			return -EINVAL;
		}
		...
		if (!strcmp(ptr_param_data->pull_V, "PUP_VOLT")) {
			...
			kfree(ptr_param_data);
			mtktsusb_printk(
					"[mtktsusb_write] bad PUP_VOLT argument\n");
			return -EINVAL;
		}
		...
		if (!strcmp(ptr_param_data->overcrilow, "OVER_CRITICAL_L")) {
			...
			kfree(ptr_param_data);
			mtktsusb_printk(
					"[mtktsusb_write] bad OVERCRIT_L argument\n");
			return -EINVAL;
		}
		...
		if (!strcmp(ptr_param_data->NTC_TABLE, "NTC_TABLE")) {
			...
			kfree(ptr_param_data);
			mtktsusb_printk(
					"[mtktsusb_write] bad NTC_TABLE argument\n");
			return -EINVAL;
		}
		...
		mtktsusb_prepare_table(g_RAP_ntc_table);

		kfree(ptr_param_data);
		return count;
	}

	mtktsusb_printk("[mtktsusb_write] bad argument\n");
	kfree(ptr_param_data);
	return -EINVAL;
}
...
static int mtktsusb_register_thermal(void)
{
	...
	/* trips : trip 0~1 */
	thz_dev = mtk_thermal_zone_device_register("mtktsflash", trips, num_trip,
					NULL, &mtktsusb_dev_ops, 0, 0, 0,
					interval * 1000);
...
static int mtktsusb_param_open(struct inode *inode, struct file *file)
{
	return single_open(file, mtktsusb_param_read, NULL);
}

static const struct proc_ops mtktsusb_param_fops = {
	.proc_open = mtktsusb_param_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_write = mtktsusb_param_write,
	.proc_release = single_release,
};


#if IS_ENABLED(CONFIG_DEVICE_MODULES_MEDIATEK_MT6577_AUXADC)
static int mtktsusb_probe(struct platform_device *pdev)
{
	...
	if (!pdev->dev.of_node) {
		mtktsusb_printk("[%s]Only DT based supported\n",
		__func__);
		return -ENODEV;
	}
	...
	if (ret) {
		mtktsusb_printk("[%s] fail to get auxadc iio ch3: %d\n",
			__func__, ret);
		return ret;
	}
...
static struct platform_driver mtk_thermal_tsusb_driver  = {
	.remove = NULL,
	.shutdown = NULL,
	.probe = mtktsusb_probe,
	.suspend = NULL,
	.resume = NULL,
	.driver = {
...
#if IS_ENABLED(CONFIG_DEVICE_MODULES_MEDIATEK_MT6577_AUXADC)
	err = platform_driver_register(&mtk_thermal_tsusb_driver );
	if (err) {
		mtktsusb_printk("thermal driver callback register failed.\n");
		return err;
	}
#else
	/* setup default table */
