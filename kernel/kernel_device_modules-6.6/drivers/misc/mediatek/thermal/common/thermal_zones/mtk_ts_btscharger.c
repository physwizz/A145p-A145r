...
pr_info("[Thermal/tzcharger]" fmt, ##args)
...
/* #define INPUT_PARAM_FROM_USER_tzcharger*/
...
static __s32 mtktscharger_thermistor_conver_temp(__s32 Res)
{
	int i = 0;
	int asize = 0;
	__s32 RES1 = 0, RES2 = 0;
...
static __s32 mtk_ts_btscharger_volt_to_temp(__u32 dwVolt)
{
	...
	/* convert register to temperature */
	BTSCHARGER_TMP   = mtktscharger_thermistor_conver_temp(TRes);

	return BTSCHARGER_TMP  ;
}

static int get_hw_tscharger_temp(void)
{
#if IS_ENABLED(CONFIG_DEVICE_MODULES_MEDIATEK_MT6577_AUXADC)
	int val = 0;
	int ret = 0, output;
	...
	if (IS_ERR_OR_NULL(thermistor_ch2)) {
		mtktscharger_dprintk("invalid thermistor_ch2:0x%p\n", thermistor_ch2);
		return ret;
	}
	ret = iio_read_channel_processed(thermistor_ch2, &val);
	mtktscharger_dprintk("%s val=%d\n", __func__, val);

	if (ret < 0) {
		mtktscharger_printk("IIO channel read failed %d\n", ret);
		return ret;
	}
	...
	mtktscharger_dprintk("mtktscharger output temperature = %d\n", output);
	return output;
}

static DEFINE_MUTEX(BTSCHARGER_lock);
/*int ts_mtktscharger_at_boot_time = 0;*/
int mtktscharger_get_hw_temp(void)
{
	int t_ret = 0;

	mutex_lock(&BTSCHARGER_lock);

	/* get HW AP temp (TSAP) */
	/* cat /sys/class/power_supply/AP/AP_temp */
	t_ret = get_hw_tscharger_temp();
#ifndef APPLY_PRECISE_BTS_TEMP
	t_ret = t_ret * 1000;
#endif

	mutex_unlock(&BTSCHARGER_lock);

	if (t_ret > 40000)	/* abnormal high temp */
		mtktscharger_printk("T_mtktscharger=%d\n", t_ret);

	mtktscharger_dprintk("[%s] T_mtktscharger, %d\n", __func__,
									t_ret);
	return t_ret;
}
...
static int mtktscharger_bind(struct thermal_zone_device *thermal,
			      struct thermal_cooling_device *cdev)
{
	...
	mtktscharger_dprintk("[%s] binding OK, %d\n", __func__,
								table_val);
	return 0;
}
...
static int mtktscharger_change_mode(struct thermal_zone_device *thermal,
...
struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	...
	if (sscanf
		...
		&num_trip,
		&ptr_mtktscharger_data->trip[0], &ptr_mtktscharger_data->t_type[0],
		ptr_mtktscharger_data->bind0,
		&ptr_mtktscharger_data->trip[1], &ptr_mtktscharger_data->t_type[1],
		ptr_mtktscharger_data->bind1,
		&ptr_mtktscharger_data->trip[2], &ptr_mtktscharger_data->t_type[2],
		ptr_mtktscharger_data->bind2,
		&ptr_mtktscharger_data->trip[3], &ptr_mtktscharger_data->t_type[3],
		ptr_mtktscharger_data->bind3,
		&ptr_mtktscharger_data->trip[4], &ptr_mtktscharger_data->t_type[4],
		ptr_mtktscharger_data->bind4,
		&ptr_mtktscharger_data->trip[5], &ptr_mtktscharger_data->t_type[5],
		ptr_mtktscharger_data->bind5,
		&ptr_mtktscharger_data->trip[6], &ptr_mtktscharger_data->t_type[6],
		ptr_mtktscharger_data->bind6,
		&ptr_mtktscharger_data->trip[7], &ptr_mtktscharger_data->t_type[7],
		ptr_mtktscharger_data->bind7,
		&ptr_mtktscharger_data->trip[8], &ptr_mtktscharger_data->t_type[8],
		ptr_mtktscharger_data->bind8,
		&ptr_mtktscharger_data->trip[9], &ptr_mtktscharger_data->t_type[9],
		ptr_mtktscharger_data->bind9,
	...
	aee_kernel_warning_api(__FILE__, __LINE__, DB_OPT_DEFAULT,
					"mtktscharger_write", "Bad argument");
	#endif
	kfree(ptr_mtktscharger_data);
	return -EINVAL;
}
...
void mtktscharger_prepare_table(int table_num)
{
	...
	pr_notice("[Thermal/tzcharger] %s table_num=%d\n",
						__func__, table_num);
...
static ssize_t mtktscharger_param_write(
struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	int len = 0;
	struct param_data {
	...
	if (sscanf
		...
		if (!strcmp(ptr_param_data->pull_R, "PUP_R")) {
			...
			kfree(ptr_param_data);
			mtktscharger_printk(
					"[mtktscharger_write] bad PUP_R argument\n");
			return -EINVAL;
		}
		...
		if (!strcmp(ptr_param_data->pull_V, "PUP_VOLT")) {
			...
			kfree(ptr_param_data);
			mtktscharger_printk(
					"[mtktscharger_write] bad PUP_VOLT argument\n");
			return -EINVAL;
		}
		...
		if (!strcmp(ptr_param_data->overcrilow, "OVER_CRITICAL_L")) {
			...
			kfree(ptr_param_data);
			mtktscharger_printk(
					"[mtktscharger_write] bad OVERCRIT_L argument\n");
			return -EINVAL;
		}
		...
		if (!strcmp(ptr_param_data->NTC_TABLE, "NTC_TABLE")) {
			...
			kfree(ptr_param_data);
			mtktscharger_printk(
					"[mtktscharger_write] bad NTC_TABLE argument\n");
			return -EINVAL;
		}
...
static int mtkts_charger_register_thermal(void)
{
	...
	/* trips : trip 0~1 */
	thz_dev = mtk_thermal_zone_device_register("mtktscharger", trips, num_trip,
					NULL, &mtktscharger_dev_ops, 0, 0, 0,
					interval * 1000);
...
static const struct proc_ops mtktscharger_param_fops = {
	.proc_open = mtktscharger_param_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_write = mtktscharger_param_write,
	.proc_release = single_release,
};


#if IS_ENABLED(CONFIG_DEVICE_MODULES_MEDIATEK_MT6577_AUXADC)
static int mtktscharger_probe(struct platform_device *pdev)
{
	...
	if (!pdev->dev.of_node) {
		mtktscharger_printk("[%s]Only DT based supported\n",
		__func__);
		return -ENODEV;
	}
	...
	if (ret) {
		mtktscharger_printk("[%s] fail to get auxadc iio ch2: %d\n",
			__func__, ret);
		return ret;
	}
...
static struct platform_driver mtk_thermal_mtktscharger_driver = {
	.remove = NULL,
	.shutdown = NULL,
	.probe = mtktscharger_probe,
	.suspend = NULL,
	.resume = NULL,
	.driver = {
...
#if IS_ENABLED(CONFIG_DEVICE_MODULES_MEDIATEK_MT6577_AUXADC)
	err = platform_driver_register(&mtk_thermal_mtktscharger_driver);
	if (err) {
		mtktscharger_printk("thermal driver callback register failed.\n");
		return err;
	}
#else
	/* setup default table */
