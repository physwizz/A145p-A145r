...
static int mtktscharger2_register_thermal(void)
{
	...
	/* trips : trip 0~2 */
	thz_dev = mtk_thermal_zone_device_register("mtktscharger2",
				trips,
				num_trip,
...
struct thermal_cooling_device *cdev, unsigned long state)
{
	...
	if (cl_dev_sysrst_state == 1) {
		 ...
		 */
		/*A06_V code for  SR-AL7160V-01-122 by yexuedong at 20240929 start*/
		#if (!IS_ENABLED(CONFIG_HQ_PROJECT_O8)) && (!IS_ENABLED(CONFIG_HQ_PROJECT_O22))
		BUG_ON(1);
		#endif
		/*A06_V code for  SR-AL7160V-01-122 by yexuedong at 20240929 end*/
	}
